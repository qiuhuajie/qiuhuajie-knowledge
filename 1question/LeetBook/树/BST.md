...
- [[#2. 例题]]
    ...
    - [[#📑剑指 Offer 36. 二叉搜索树与双向链表⚔️]]
    - [[#📑剑指 Offer 54. 二叉搜索树的第k大节点⚔️]]
    - [[#📑剑指 Offer 68 - I. 二叉搜索树的最近公共祖先⚔️]]
# 1. BST 介绍
1. 二叉树算法的设计总路线：**==明确一个节点要做的事，然后剩下的事交给递归框架==**
1. 二叉搜索树（BST）是二叉树的一种特殊表示形式，它满足如下特性：
    
    1. 每个节点中的值必须**大于**存储在其左侧子树中的任何值
    
    1. 每个节点中的值必须**小于**存储在其右子树中的任何值
    
1. 下面是一个二叉搜索树的例子：
    
    ![[IMG-20260405035400261.png|Untitled 85.png]]
    
1. 像普通的二叉树一样，我们可以按照前序、中序和后序来遍历一个二叉搜索树
    
    1. BST **==中序遍历==**的特点
        
        1. **对于二叉搜索树，**==**可以通过中序遍历得到一个递增的有序序列**==
        
        1. 因此，中序遍历是二叉搜索树中最常用的遍历方法
    ...
    1. BST **==后序遍历==**的特点
        ...
        1. 数组中==前面的数字可以分为====**两部分**====：第一部分是左子树的元素（都比根节点小）；第二部分是右子树的元素（都比根节点大）==
            
            ![[IMG-20260405035401660.png|Untitled 1 53.png]]
            
        
        1. 序列中不可能存在 `3，1，2` 模式的子序列
...
1. 示例：
    ...
    ![[IMG-20260405035404232.png|Untitled 2 42.png]]
...
1. 示例：
    ...
    [![](https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2019/05/03/tree.png)](https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2019/05/03/tree.png)
...
1. 示例：
    ...
    ![[IMG-20260405035405658.png|Untitled 3 34.png]]
