...
3. 官网地址：
    > ℹ️ RestTemplate (Spring Framework 5.2.2.RELEASE API)
    > NOTE: As of 5.
...
4. 使用：
    ...
    2. `(url, requestMap, ResponseBean.class)`这三个参数分别代表 REST请求地址、请求参数、HTTP响应转换被转换成的对象类型
5. **RestTemplate 提供的方法接口：**
    1. **getForObject()：（常用）**

        返回对象为响应体中数据转化成的对象，基本上**可以理解为Json串**

        ![[IMG-20260405035535187.png|800]]

    2. **getForEntity()：**

        返回对象为ResponseEntity对象，**包含了响应中的一些更详细的重要信息**，比如响应头、响应状态码、响应体等

        ![[IMG-20260405035539963.png|800]]

    3. **postForObject()：**
    4. **postForEntity()：**

        ![[IMG-20260405035540014.png|800]]

# 二、参考资料
> ℹ️ RestTemplate 用法详解_决战灬的博客-CSDN博客_resttemplate方法介绍
