...
1. BlockingQueue 通常**用于一个线程生产对象，而另外一个线程消费这些对象的场景**
    ...
    ![[IMG-20260404031738238.png|Untitled 304.png]]
...
1. 这样的阻塞队列可以应对生产者和消费者之间**数据处理速度不匹配的问题**
    
    1. **生产者会持续生产新对象并将其插入到队列之中**
        
        1. 如果该阻塞队列到==达了其临界点==，负责生产的线程将会在往里边插入新对象时发生==阻塞==
        
        1. 它会一直处于阻塞之中，直到负责消费的线程从队列中拿走一个对象
        
    
    1. **消费者将会一直从该阻塞队列中拿出对象**
...
1. **阻塞队列 BlockingQueue 种类：（**==**在**==[[ThreadPoolExecutor]]==**提供的几种线程池部分有应用**==**）**
    

    ![[IMG-20260404031738308.png|Untitled 1 230.png]]
...
1. 因此，**BlockingDeque 可以从任意一端插入或者抽取元素的队列**
    
    1. 且在不能够插入元素时，它将==阻塞==住试图插入元素的线程
    
    1. 在不能够抽取元素时，它将==阻塞==住试图抽取的线程
    

    ![[IMG-20260404031738352.png|Untitled 2 194.png]]
...
1. **使用场景**
    ...
    1. 如果生产者线程需要在队列的两端都可以插入数据，消费者线程需要在队列的两端都可以移除数据，这个时候也可以使用 BlockingDeque
    
1. **BlockingDeque 接口继承自 BlockingQueue 接口**。这就意味着你可以像使用一个 BlockingQueue 那样使用 BlockingDeque
1. 种类
