...
1. **Fork/Join** ==**在分治的基础上加入了多线程**==**，可以**==**把每个任务的分解和合并交给不同的线程来完成**==**，进一步提升了运算效率**
    
    1. 所谓的任务拆分，是将一个大任务拆分为算法上相同的小任务，直至不能拆分可以直接求解
    
    1. 跟递归相关的一些计算，如归并排序、斐波那契数列、都可以用分治思想进行求解
    
1. ==**适用于能够进行任务拆分的 cpu 密集型运算**==
1. Fork/Join 默认会创建与 cpu 核心数大小相同的线程池 `ForkJoinPool`
    ...
    ![[IMG-20260404031742111.png|image 8.png]]

    
1. Fork/Join框架主要包含三个模块：
