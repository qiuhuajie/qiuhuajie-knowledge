...
- [[#四、Threadlocal 在项目中的使用]]
- [[#五、InheritableThreadLocal🙋‍♂]]
    - [[#1. Threadlocal 的缺点]]
    - [[#2. InheritableThreadLocal]]
    - [[#3. InheritableThreadLocal 缺陷]]

# 一、介绍
1. 通常情况下，创建的变量是可以被任何一个线程访问并修改的
2. **如何实现**==**每一个线程都有自己的专属本地变量副本**==**，避免两个线程之间竞争数据，从而**==**避免了线程安全**==**问题**
    1. ThreadLocal 类主要解决的就是让每个线程绑定自己的值
    2. 可以将 ThreadLocal 类形象的比喻成存放数据的盒子，盒子中可以存储每个线程的私有数据
        1. 创建一个 Threadlocal 变量，那么访问这个变量的==每个线程都会有这个变量的本地副本==
        2. 各个线程可以使用 **`get()`** 和 **`set()`** 方法来获取默认值或将其值更改为当前线程自己的值
        ![[IMG-20260619223939529.png|516]]

    > **ThreadLocal 是否会存在另一个线程修改当前线程数据的问题**
3. 🙋‍♂️ **两台机器如何实现 ThreadLocal 信息共享？**
    1. 可以使用分布式缓存来存储 ThreadLocal 变量的值
        1. 例如，可以使用 **Redis 等分布式缓存**来存储 ThreadLocal 变量的值，并使用一个**全局唯一的标识符**来标识每个 ThreadLocal 变量
        2. 然后，在不同的机器上的线程可以通过该标识符从分布式缓存中获取相应的值，并在需要的时候将其设置为当前线程的 ThreadLocal 变量的值
    2. 另外一种可能的方法是使用**远程过程调用（RPC）**来实现 ThreadLocal 信息的共享
    3. 无论使用哪种方法，都需要考虑到线程安全和性能问题，以及不同机器之间的数据一致性和可靠性
4. **ThreadLocal 与 Synchronized 的比较**
    1. 同：
...
1. 在 ThreadLocal 中有一个静态内部类 **`ThreadLocalMap`**
    ...
    ![[IMG-20260619223939623.png|551]]

2. ==**每个 Thread 中都具备一个 ThreadLocalMap ，而 ThreadLocalMap 可以存储以 Threadlocal 为 key ，Object 对象为 value 的 Entry 键值对**==

    ![[IMG-20260619223939706.png|659]]

    1. ThreadLocal 对象的 `set()` 方法在设置值时，会结果把 ThreadLocal 对象自己当做 Key，放进了 ThreadLoalMap中

        ![[IMG-20260619223939787.png|333]]

    2. ThreadLocal 对象的 `get()` 方法在获取值时，会去当前线程的 Map 里面找，Key 为当前 ThreadLocal 对应的 Value

        ![[IMG-20260619223939819.png|448]]

3. **==Entry 键值==**==对就是==**==每个线程==**==根据共享内存中的 ThreadLocal 变量所维护的一个==**==属于线程自己的==** **==ThreadLocal 变量的副本==**
# 三、ThreadLocal 内存泄露问题
...
1. ==**首先，要保证 ThreadLocal 对象本身需要被正确回收**==
    ...
    2. ==源码中，ThreadlocalMap 中 key 是 WeakReference 类型==

        ![[IMG-20260619223939869.png|509]]

    3. 图示：

        ![[IMG-20260619223939906.png|577]]
        ...
        - ==如果 key 指向 ThreadLocal 对象的引用也是一个强引用的话==
            ...
            2. 也即可能一直存在一条强引用链：Thread 引用 ➡️ Thread 对象 ➡️ ThreadLocalMap ➡️ Entry 的 Key ➡️ ThreadLocal 对象
            3. 这时，即使 ThreadLocal 引用 ➡️ ThreadLocal 对象的强引用被删除，ThreadLocal 对象也永远不会被删除
        - 所以，==key 指向 ThreadLocal 对象的引用要设置成一个软引用，当 ThreadLocal 引用 ➡️ ThreadLocal 对象的强引用被删除，ThreadLocal 对象就会被删除==
2. ==**其次，要保证在 ThreadLocal 对象被删除后，ThreadLocalMap 中对应的 Entry 也被正确清理**==
    1. 存在的问题：ThreadLocal 对象被删除后，此时 Entry 的 Key 就会变为一个 null，如果不做任何处理，这些 null ↔️ Value 的键值对就永远无法被清理，造成业务数据污染 + 内存泄漏
    2. 所以，==必须在代码 try-finally 块中，主动调用== ==`remove()`== ==清理 ThreadLocal 对象==

        ![[IMG-20260619223939936.png|619]]
...
1. 将业务的一些==**上下文信息**==，保存在 ThreadLocal 对象中，实现上下文信息和线程的关联关系
    ...
    2. **登录校验：**[[校验登录状态]]
        ...
        2. 如果已经登录，将用户信息放在 Threadlocal 中，这样 Controller 就可以拿到当前用户的信息了
        3. 不过引入 Redis 可以代替这种方式
2. 如果有**很多变量**都要塞到 ThreadlocalMap 中，那岂不是要申明很多个Threadlocal 对象？
...
1. **Threadlocal 的缺点：**==**不能在父子线程中线程之间传递**==
    1. 演示：
        ![[IMG-20260619223939990.png|535]]
    2. 结果：**子线程无法访问父线程中设置的本地变量**
        ![[IMG-20260619223940038.png|328]]
## 2. InheritableThreadLocal
...
2. 实现原理：
    ...
    2. `InheritableThreadLocal#getMap`
        ![[IMG-20260619223940079.png|311]]
    3. `Thread#init` ⭐
        ![[IMG-20260619223940118.png|606]]
    ...
    4. 子线程默认拷贝父线程的方式是**浅拷贝**，如果需要使用深拷贝，需要使用自定义 ThreadLocal，继承 InheritableThreadLocal 并重写 `childValue` 方法
...
1. ==**在配合线程池使用时，会出现本地变量混乱的现象**==
    - 代码演示：
        ...
        ```
2. 结果：线程7 的变量值为 6，但其子线程却以为自己的父线程的变量值为 7
    ![[IMG-20260619223940163.png|387]]
3. 原因：
    1. 线程池能够复用线程，减少线程的频繁创建与销毁
    2. 如果使用 `InheritableThreadLocal`，那么线程池中的线程拷贝的数据来自于第一个提交任务的外部线程
    3. 由于线程池的**复用**机制，“子线程”只会复制一次，即后面的外部线程向线程池中提交任务时，**子线程访问的本地变量始终都来源于第一个外部线程**，造成线程本地变量混乱
    4. 在全链路跟踪与压测出现这种情况是致命的
4. 解决：
...
* https://github.com/alibaba/transmittable-thread-local
    3. 依赖：
        ...
        ```
    4. `TransmittableThreadLocal` 实现原理
        1. 在父线程向线程池提交任务时复制父线程的上下环境
        2. 那在子线程中就能够如愿访问到父线程中的本地变量，实现本地环境变量在线程池调用中的透传，从而为实现链路跟踪打下坚实的基础