...
1. CountdownLatch 其底层是由 AQS 提供支持的一个同步器锁，相对简单，更像是一个工具锁，用来简化实现线程间的通讯
    ...
    ![[IMG-20260404031737214.png|Untitled 307.png]]
    
1. 使用 CountDownLatch 的好处
...
1. ==**使用场景：**==
    ...
    1. 用在**子任务和结果汇总任务之间的通讯**
        ...
        1. 当每一个任务完成时，都会在这个锁存器上**调用 `countDown()`，让计数器 `-1`**
        
        1. 汇总 n 个子任务的代码**调用这个锁存器的 `await()`，将他们自己拦住**
        
        1. **直至锁存器计数减至 `0`，这个汇总任务会被唤醒，即可开始结果汇总**
        
    
    1. **在使用线程池时，线程会被复用，使用 CountDownLatch 可以用来判断提交的任务是否已经执行完成**[[ThreadPoolExecutor]]
