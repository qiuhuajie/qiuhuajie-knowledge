...
2. 这个类在 java.util.concurrent.locks 包下面，是**整个 JUC 体系的基石**
3. **AQS 是一个**==**用来构建锁和同步器的框架**==
    ...
    2. 也能利用 AQS 非常轻松容易地构造出符合我们自己需求的同步器
4. AQS 核心思想是
    1. 如果被请求的共享资源空闲，则将当前请求资源的线程设置为有效的工作线程，并且将共享资源设置为锁定状态
    2. 如果被请求的共享资源被占用，那么就需要一套线程阻塞等待以及被唤醒时，锁分配的机制
    3. 这个机制 AQS 是用一个 CLH 双向队列锁实现的，即将暂时获取不到锁的线程加入到队列中
5. **AQS 底层使用了模板模式**[[模板模式 ⭐]]
6. **AQS定义两种资源共享方式**
    - Exclusive（独占）：只有一个线程能执行，如 ReentrantLock。又可分为公平锁和非公平锁：
        - 公平锁：按照线程在队列中的排队顺序，先到者先拿到锁
        - 非公平锁：当线程要获取锁时，无视队列顺序直接去抢锁，谁抢到就是谁的
    - Share（共享）：多个线程可同时执行，如Semaphore、CountDownLatCh、 CyclicBarrier、ReadWriteLock 我们都会在后面讲到
7. 🙋‍♂️ **AQS 和 synchronized 的主要区别在于（都是Java中的同步机制）：**
    1. **实现方式：**
        1. AQS是一个抽象类，它提供了一个框架来实现同步器，开发人员需要继承AQS并实现其抽象方法来实现自己的同步器
        2. 而synchronized是Java语言提供的一种关键字，用于控制多个线程对共享资源的访问
    2. **粒度：**
        1. AQS可以实现更细粒度的同步控制，例如独占锁和共享锁
        2. 而synchronized只能实现粗粒度的同步控制，它只能将一个方法或代码块作为同步块
    3. **性能：**
        1. 在高并发情况下，AQS的性能通常比synchronized更好。因为AQS使用基于CAS（Compare-And-Swap）操作的无锁算法来实现同步
        2. 而synchronized使用的是重量级锁，它需要进行上下文切换和内核态与用户态之间的切换

# 二、AQS 数据结构

**状态标志位** `state` **➕ 一个虚拟的 FIFO 双向队列**

![[IMG-20260619222626078.png|569]]

![[IMG-20260619222626160.png|549]]
...
1. **使用一个 `volatile` `int` 成员变量 `state` 来表示共享资源同步状态**
    ...
    ```
2. **AQS 使用 CAS 对该同步状态进行原子操作实现对其值的修改**：通过 procted 类型的 `getState()`，`setState()`，`compareAndSetState()` 进行操作状态信息
...
2. AQS 将**每条请求共享资源的线程**封装成一个 CLH 锁队列的一个结点 **`Node`**，来完成获取资源线程的排队工作，进行锁的分配
3. **head 指向的第一个节点是一个**==**哨兵节点**==**，用来占位，不关联线程**
...
1. 该方法以独占模式获取(资源)，忽略中断，即线程在aquire过程中，中断此线程是无效的
    ...
    ```
2. 如果执行 `tryAcquire()` 尝试加锁不成功 返回 false，则会执行 `acquireQueued(addWaiter(Node.EXCLUSIVE), arg)` 将当前线程放入等待队列
3. `tryAcquire()` **方法在 AQS 类中是一个空实现，需要继承了 AQS 的同步器子类重写这个方法（详见 下面的 `MySync` ）**
![[IMG-20260619222626237.png|412]]
## 4. 释放锁方法：release()
1. 以独占模式释放对象，其源码如下
    ...
    ```
2. 其中，`tryRelease()` 的默认实现是抛出异常，需要具体的子类实现
3. 如果 `tryRelease()` 成功，那么如果头节点不为空并且头节点的状态不为 0，则**使用 `unparkSuccessor()` 释放头节点的后继结点**
4. `tryRelease()` **方法在 AQS 类中同样是一个空实现，需要继承了 AQS 的同步器子类重写这个方法（详见 下面的 `MySync` ）**
![[IMG-20260619222626314.png|399]]
# 四、使用 AQS 自定义一个独占锁
1. 实现一个锁需要很多方法，如果自己实现太难了
2. 但是，如果**使用一个继承自 `AbstractQueuedSynchronizer` 的同步器类 `MySync`，同步器里面已经把大部分方法都实现好了**，这样我们实现一个自定义锁就变得很简单了
