...
- [[#三、LockSupport 的使用]]
- [[#四、比较]]
    - [[#1. Thread.sleep() 和 LockSupport.park() 的区别]]
    - [[#2. Object.wait() 和 LockSupport.park() 的区别]]

# 一、LockSupport 介绍
1. **LockSupport 是**==**用来创建锁和其他同步类**==**的**==**基本线程阻塞原语**==
    1. LockSupport 类使用了一种名为 **Permit（许可）的概念**来做到阻塞和唤醒线程的功能
    2. **一个线程最多只会有一个许可**，即许可只有两个值 `1` 和 `0`，默认是 `0`
2. **在**==**使用 LockSupport 实现线程的等待和唤醒时，没有限制条件，更加灵活**==（详见 下 LockSupport）
# 二、LockSupport API
## 1. Park 函数
1. **`park()`**：
    1. 调用后阻塞线程，除非许可可用，或 该线程被中断（比如其他线程调用 `interrupt()`）
    2. 由于许可默认为 0，**当调用 `park()` 后当前线程就会阻塞，直到别的线程将当前线程的许可设置为 1 时，`park()` 方法会被唤醒，然后会将许可再次设置为 0 并返回**
2. **`park(boolean isAbsolute, long time)`**
    ...
    2. 参数说明：
        ...
        2. 当 time 为 0 时，表示无限等待，直到 unpark() 发生
3. **`park(Object blocker)`**
4. **`parkNanos(Object blocker, long nanos)`**：在许可可用前阻塞当前线程，并最多等待指定的等待时间
5. **`parkUntil(Object blocker, long deadline)`**：在指定的时限前禁用当前线程，除非许可可用
## 2. Unpark 函数
1. **`unpark(Thread thread)`**：
    1. 如果给定线程尚不可用，则为其提供许可
    2. **调用 `unpark(thread)` 方法后，就会将指定的 thread 线程的许可设置成 1，会自动唤醒 thread 线程，即之前阻塞中的 `LockSupport.park()` 方法会立即返回**
    3. 注意：多次调用 `unpark()` 方法，**许可数量不会累加**，一个线程的许可数始终为 1
...
2. LockSupport：
    1. **`wait()`** + `**`notify()`**`、**`await()`** + **`signal()`** 的方法在实现等待和唤醒时，会有限制条件
        ...
        3. 且这些方法在声明时，抛出了 `InterruptedException` 中断异常，所以在使用时需要捕获异常
    2. ==⭐====**但 LockSupport 实现等待和唤醒时，没有限制条件，更加灵活**==
        1. **不需要在锁块中使用**
        2. **即使 `unpark(t1)` 在 `park()` 之前执行，线程的等待和唤醒操作依然可以顺利执行**
        3. **且不需要捕获异常**
        4. **线程被唤醒后，一定会继续执行（被唤醒后不需要再去竞争锁）**
3. 运行示例：
