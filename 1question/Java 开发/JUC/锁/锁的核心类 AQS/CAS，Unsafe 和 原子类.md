...
1. CAS 的全称为 Compare-And-Swap，直译就是对比交换
    ...
    2. CAS 是**靠硬件实现**的
        ...
        2. JVM 只是封装了汇编调用，那些原子类，如 `AtomicInteger` 类便是使用了这些封装后的接口
2. **==CAS 更新操作的流程==**
    1. 它包含三个操作数：**内存位置值 V，旧的预期值 A，要修改的更新值 B**
    2. 执行 CAS 操作的时候，将内存位置值 V 与旧的预期值 A 比较：
        1. 如果相匹配，那么处理器会自动将内存位置值 V 更新为要修改的更新值 B
        2. 如果不匹配，处理器不做任何操作，进入自旋操作（`do-while` 重试）
        ![[IMG-20260619222631295.png|443]]
3. **CAS 必须配合 `volatile` ，只有保证可见性，才能在 CAS 比较时拿到最新的值**
## 2. CAS 使用示例
1. `i++` 分为三步：取出，加一，放回，这样在多线程环境下自增操作存在并发安全问题
2. **如果不使用 CAS**，在高并发下，多线程同时修改一个变量的值我们需要 **`synchronized`** 加重量级锁（可能有人说可以用 Lock 加锁，Lock 底层的 AQS 也是基于 CAS 进行获取锁的）
    ...
    ```
3. Java 并发包中提供了 **AtomicInteger 原子类（底层基于 CAS 进行更新数据的）**，**==不需要加锁==**就在多线程并发场景下实现数据的一致性
...
1. ==**ABA 问题**== 🙋‍♂️
    ...
    2. 解决方法：
        ...
        2. 从Java 1.5开始，**JDK 的 Atomic 包里提供了一个引用类型的原子类** **`AtomicStampedReference`** **来解决 ABA 问题**
        3. 这个类的 `compareAndSet()` 方法的作用是
            1. 首先检查当前引用是否等于预期引用，**并且检查当前标志是否等于预期标志**
            2. 如果全部相等，则以原子方式将该引用和该标志的值设置为给定的更新值
2. ==**重试存在开销**==
    1. 如果 CAS 失败，会一直进行尝试
    2. 自旋 CAS 如果长时间不成功，会给 CPU 带来非常大的执行开销
3. ==**只能保证一个共享变量的原子操作**==
    ...
    2. 解决方法：
        ...
        2. 还有一个取巧的办法，就是把多个共享变量合并成一个共享变量来操作。比如，有两个共享变量i = 2，j = a，合并一下ij = 2a，然后用CAS来操作ij

# 二、UnSafe 类
1. Unsafe 是位于 sun.misc 包下的一个类
2. Unsafe 作用
    1. **主要==提供一些用于执行低级别、不安全操作的方法==**
    2. **Unsafe 提供的 API 大致可分为：内存操作、CAS、Class 相关、对象操作、线程调度、系统信息获取、内存屏障、数组操作等几类**
    3. 这些方法**可以==提升 Java 运行效率、增强 Java 语言操作底层资源的能力==**
    4. 例如：⭐**Unsafe 类源码中提供了三种 CAS 方法，都是** `native` **方法**
        ![[IMG-20260619222631380.png|797]]
3. **Unsafe 使用有安全风险**
    1. 但由于 Unsafe 类使 Java 语言拥有了类似 C 语言指针一样操作内存空间的能力，这无疑也增加了程序发生相关指针问题的风险
    2. 在程序中过度、不正确使用 Unsafe 类会使得程序出错的概率变大，使得 Java 这种安全的语言变得不再“安全”，因此对 Unsafe 的使用一定要慎重
    3. 对于 Unsafe 类的使用都是受限制的，只有授信的代码才能获得该类的实例，当然 JDK 库里面的类是可以随意使用的
...
1. ==**AtomicInteger 底层用的是**== **`volatile`** ==**的变量和 CAS 来进行更改数据的**==
    ...
    ```
2. 以递增方法 **`incrementAndGet()`** 为例，调用 Unsafe 类的 **`getAndAddInt()`** 方法

    ![[IMG-20260619222631456.png|637]]

3. Unsafe 类的 `getAndAddInt()` 方法中**使用 `do-while` 实现自旋**，如果修改数值失败则通过循环来执行自旋，直至修改成功
4. **而修改更新数据的操作使用的就是 CAS，可以看到调用了 Unsafe 类提供的三个 CAS 方法之一的 `compareAndSwapInt()`**
## 2. 其他原子类
> JDK 中提供了 12 个原子操作类
### 2.1 A. 基本类型原子类
...
2. 这三个类提供的方法都差不多
    ...
    2. 但 AtomicReferenceFieldUpdater 略有不同，更新的字段必须用 volatile 修饰
3. AtomicReference 代码示例：
    ...
    p3.equals(p1) = false
    ```
4. ==⭐====**AtomicStampedReference 解决 CAS 的 ABA 问题**==
...
2. 性能提升的原因很简单
    ...
    2. 最后将结果汇总
    3. 这样它们在**累加时操作的不同的 Cell 变量，因此减少了 CAS 重试失败，从而提高性能**
