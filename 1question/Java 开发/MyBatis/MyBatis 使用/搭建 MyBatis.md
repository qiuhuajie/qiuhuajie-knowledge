...
<aside>
📌 **整合Spring之后，这个配置文件可以省略**

</aside>

1. 习惯上命名为 **mybatis-config.xml**，这个文件名仅仅只是建议，并非强制要求
2. 核心配置文件主要**用于配置连接数据库的环境以及MyBatis的全局配置信息**
3. 核心配置文件存放的位置是`src/main/resources`目录下
...
2. 但是区别在于，`mapper`仅仅是接口，**不需要提供实现类**
3. **MaBatis中由面向接口编程的功能，每当调用接口中的方法时，会自动匹配一个 SQL 语句，然后执行**
...
1. 映射文件介绍
    ...
    2. 相关概念：ORM（Object Relationship Mapping）对象关系映射
        1. 对象：Java的实体类对象
        2. 关系：关系型数据库
        3. 映射：二者之间的对应关系
    3. **一个映射文件对应一个实体类，对应一张表的操作**
    4. MyBatis映射文件用于编写SQL，访问以及操作表中的数据
    5. MyBatis映射文件存放的位置是 `src/main/resources/mappers` 目录下
2. ⭐**映射文件中的需要遵循的规则**
    1. 映射文件的命名规则：
    ...
    2. MyBatis中可以面向接口操作数据，要保证两个一致：
        ...
        2. **mapper接口中方法的方法名** 和 **映射文件中编写SQL的标签的id属性**保持一致
...
2. **SqlSessionFactory**：是生产 SqlSession 的工厂
...
4. 🔴通过 **代理模式** 创建 UserMapper 接口的代理实现类对象：
...
2. 在 resource 目录下添加配置文件：log4j.xml
...
3. 再次执行 测试方法：
    ...
    ![Untitled](IMG-20260621001348431.png)