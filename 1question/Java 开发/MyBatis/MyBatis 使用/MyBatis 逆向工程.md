...
2. **逆向工程：**先创建数据库表，由框架负责**根据数据库表，反向生成如下资源**：
    1. Java实体类
    2. Mapper接口
    3. Mapper映射文件
...
4. 执行 MBG 插件的 generate
    ...
    ![Untitled](IMG-20260621001323771.png)

5. 自动生成的目录结构：

    ![Untitled](IMG-20260621001323819.png)

# 2. Query By Criteria（**QBC） 查询**

<aside>
📌 前提：需要是 `MyBatis3`: **生成带条件的 CRUD**（奢华尊享版）

1. 需要注意的几点：
    1. 在先生成了 简洁版的后，想在使用豪华版，需要将之前生成的文件目录全部删除，在执行生成豪华版 才会更新代码
    2. 在逆向生成的目录文件中 sql 映射文件是写在 `resource` 下的 `com/atguigu/mybatis/mapper/DeptMapper.xml` 中的，所以 mybatis 全局配置文件中的 sql 映射文件位置也要改
2. 豪华版生成的目录结构：

    ![Untitled](IMG-20260621001323907.png)
...
<aside>
📌 mapper 接口中的方法，只要是有 example 就是 根据条件来操作

![Untitled](IMG-20260621001323994.png)
