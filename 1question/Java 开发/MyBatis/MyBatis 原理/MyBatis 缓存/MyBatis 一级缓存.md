...
1. 在一次 `SqlSession`（数据库会话）中，如果程序执行多次查询，且**查询条件完全相同**，**多次查询之间程序没有其他增删改操作**，则第二次及后面的查询可以从缓存中获取数据，避免走数据库
    ...
    ![[Attachment/1question/大数据/Java 开发/MyBatis/MyBatis 原理/MyBatis 缓存/IMG-20260405035413931.png|Untitled 511.png]]
    
    1. 每个`SqlSession`中持有了`Executor`，每个`Executor`中有一个**`LocalCache`**
    
    1. 当用户发起查询时，MyBatis根据当前执行的语句生成 `MappedStatement`，在**`Local Cache`**进行查询
    
    1. 如果缓存命中的话，直接返回结果给用户
    
    1. 如果缓存没有命中的话，查询数据库，结果写入**`Local Cache`**，最后返回结果给用户
    
1. **`Local Cache`** 其实是一个 HashMap 的结构
    ...
    - 如下图所示，有两个 SqlSession，分别为 `SqlSession 01` 和 `SqlSession 02`，每个 SqlSession 中都有自己的缓存，缓存是 HashMap 结构，存放的键值对
        ...
        ![[IMG-20260404031808708.png|Untitled 1 378.png]]
...
1. 使用同一个 sqlSession 测试：
    ...
    ![[IMG-20260405035420340.png|Untitled 2 308.png]]
    
1. 使用不同的 sqlSession 测试：
    ...
    ![[IMG-20260405035422059.png|Untitled 3 231.png]]
    
# 4. **一级缓存失效的场景**
1. 不同的SqlSession对应不同的一级缓存
1. 同一个SqlSession但是查询条件不同
1. 同一个SqlSession两次查询期间执行了任何一次增删改操作（此时数据库里的数据已经发生了变化）
