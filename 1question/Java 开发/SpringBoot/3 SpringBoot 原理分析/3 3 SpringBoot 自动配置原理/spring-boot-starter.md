> [!info] 如何自定义一个场景启动器springboot-starter，从零开始实现推导。_哔哩哔哩_bilibili  
...
1. 虽然不同的starter实现起来各有差异，但是他们基本上都会使用到两个相同的内容：`ConfigurationProperties` 和 `AutoConfiguration`
![[Attachment/1question/大数据/Java 开发/SpringBoot/3 SpringBoot 原理分析/3 3 SpringBoot 自动配置原理/IMG-20260405035413988.png|image 9.png]]
1. 本质上使用了 SPI 机制 ⭐
    
    1. **可以理解成 Spring 内部一套类加载机制，读取指定位置的文件配置，进行bean的自动装配**（==SPI机制==）
    
    1. **你要写一个starter给别人用，首先，要将那些别人在使用你starter时，需要自动注入的 bean 都在starter的自动化配置类里装配好，并要在约定好的位置，将自动化配置类名写入**`**spring.factories**` **文件**
    
    1. 详见[[自动配置原理]]
        
        1. `**AutoConfigurationImportSelector**` 类则是 Spring Boot 自动装配的核心类
        
        1. 这个类会通过 ClassLoader 获取 classpath 中的配置文件 `META-INF/spring.factories` 
...
> 
> ![[IMG-20260405035420368.png|image 2 6.png]]
> 
...
>     
> 
> 1. **@Import({MetaqProducerRegistrar.class})**:
>     
...
- 效果
    ...
    ![[IMG-20260405035422092.png|image 3 6.png]]
...
- 接下来，我们通过运行`mvn install`命令，将这个项目打成jar包
    ...
    ![[IMG-20260405035434264.png|image 4 4.png]]
    
- 部署到本地仓库，提供给另一个服务调用
    
    ![[IMG-20260405035445574.png|image 5 4.png]]
...
![[IMG-20260405035505333.png|image 6 4.png]]