...
2. **JDK NIO 的 Bug：**
    ...
    2. 直到 JDK 1.7 版本该问题仍旧存在，没有被根本解决
...
2. ==**Netty** 是一个 **异步、非阻塞、事件驱动** 的网络应用程序框架==，用于**快速开发可维护的高性能协议服务器和客户端**。
3. 官网：
    > ℹ️ Netty: Home
...
2. **支持多种协议和编解码方式**：Netty 内置了多种常见的协议和编解码方式。
    1. 例如 HTTP、WebSocket、TLS、SSL、JSON、Protobuf 等等，同时可以自定义解码器解决 TCP 粘包、拆包问题
    2. 这些协议和编解码方式可以直接使用，也可以进行自定义扩展
3. Netty 的架构设计具有良好的**可扩展性**。
4. 社区活跃。
5. **==性能上的优化：==**
    1. Netty 的**线程模型**采用了**主从 Reactor 模式**，实现了高效的事件处理和线程调度，提高了性能
    2. Netty 使用**零拷贝技术**，减少了由于系统调用和数据拷贝带来的 CPU 和内存的开销
    3. Netty 采用了**异步和事件驱动编程模型**，使得应用程序可以同时处理多个连接和请求，并且不会出现阻塞或死锁等问题

## 3. Netty 的使用场景
1. 作为 **RPC 框架的网络通信工具**
    1. 我们在分布式系统中，不同服务节点之间经常需要相互调用，这个时候就需要 RPC 框架了，可以使用 Netty 来实现
    2. 比如我调用另外一个节点的方法的话，至少是要让对方知道我调用的是哪个类中的哪个方法以及相关参数吧
2. 实现一个**即时通讯系统**：使用 Netty 我们可以实现一个可以聊天类似微信的即时通讯系统，这方面的开源项目还蛮多的，可以自行去 Github 找一找
3. 实现**消息推送系统**：市面上有很多消息推送系统都是基于 Netty 来做的

## 4. Netty 的组件

![[IMG-20260620092011954.png|480]]
...
* 以上可以看出，Netty 的功能、协议、传输方式都比较全，也比较强大。

## 5. Netty 版本说明
1. Netty 版本分为 Netty 3.x、Netty 4.x 和 Netty 5.x。
2. 因为 Netty 5 出现重大 Bug，已经被官网废弃了，目前**推荐使用的是 Netty 4.x 的稳定版本**。
3. 目前在官网可下载的版本有 Netty 3.x、Netty 4.0.x 和 Netty 4.1.x。
