...
1. 在启动 cloud-gateway-gateway9527 时，会发现加载了很多的 路由断言工厂
    ...
    ![[IMG-20260405035413881.png|800]]

    **Spring Cloud Gateway 包括许多内置的 Route Predicate 工厂**

2. **工作机制**：==如果客户端发送的请求满足了断言的条件，则映射到指定的路由器，就能转发到指定的服务上进行处理==
    1. 这些 Predicate 与HTTP请求的不同属性匹配
    2. 多个 Route Predicate 可以进行组合
3. 底层在创建 Route 对象时， 使用 RoutePredicateFactory 创建 Predicate 对象，Predicate 对象可以赋值给 Route
4. **断言和路由的匹配规则**：
    - ==一对多==：一个路由规则可以包含多个断言
    - ==同时满足==：如果一个路由规则中有多个断言，则需要同时满足才能匹配
    - ==第一个匹配成功==：如果一个请求可以匹配多个路由，则映射第一个匹配成功的路由
5. 常用的 Route Predicate
    - 查看
...
1. **动态路由**
    ...
    2. 但是 Gateway 的配置是不需要更新的
    3. 这里的动态指的是==微服务的集群个数、IP 和端口是动态可变的==
2. **配置项**：由ID、目标URI、一系列的断言和过滤器组成
3. **动态路由示例**
    1. 将 URL 配置为：`uri: lb://cloud-payment-service` ，即 ==lb 协议 + 注册中心上的微服务名==
    2. Gateway 会==根据注册中心注册的服务列表，找到对应服务具体的 IP + 端口，进而创建动态路由进行转发==
    3. yaml 配置文件
    ...
    ```YAML
     ...
     eureka:
       ...\#同上
    ```
    4. 测试：[http://localhost:9527/payment/lb](http://localhost:9527/payment/lb)
        - 8002、8001交替出现

    ![[IMG-20260404031837784.png|800]]

        - 也即==当路由的 uri 使用的是 lb 协议时：Gateway 可以像 Ribbon 一样 实现 负载均衡==
4. 因此**一个请求的转发过程**
    1. 客户端先将请求发送给 Nginx，然后转发到网关，网关经过断言匹配到一个路由后，将请求转发给指定 uri
    2. 这个 uri 可以配置成 微服务的名字，比如 passjava-member
    3. ==动态路由==：Gateway 从注册中心拉取注册表，就能知道服务名对应具体的 IP + 端口
    4. ==负载均衡==：如果一个服务部署了多台机器，则还可以通过负载均衡进行请求的转发
    5. 原理如下图所示：

    ![[IMG-20260405035420289.png|800]]

# 四、过滤器
1. 过滤器的作用：==规定一些你在 用服务前 和 用服务之后 要执行的操作==
    1. 在路由过滤器中可以通过 `ServerWebExchange` 拿到请求的一些属性

    ![[IMG-20260405035421998.png|800]]

    2. 因此可以通过网关过滤器，实现一些逻辑的处理，比如 ip黑白名单拦截、特定地址的拦截、Token 的合法校验等[[Gateway 项目实战]]
2. 多个过滤器也是以执行链的方式串起来，执行顺序可以指定
    ![[IMG-20260405035432325.png|429]]
3. Spring Cloud Gateway 内置了多种路由过滤器，他们都由 GatewayFilter 的工厂类来产生
4. Spring Cloud Gateway 的 Filter 有两种：
    1. GatewayFilter
    2. GlobalFilter
5. **Filter的配置示例：**
...
6. ==**示例：自定义一个全局过滤器：GlobalFilter**==
    ...
    ![[IMG-20260405035444596.png|538]]

        3. **请求不带参数：访问被拒绝**

    ![[IMG-20260405035504868.png|800]]
