...
2. **拦截请求：**
    ...
    2. 这样所有标注 `@loadBalance` 注解的 RestTemplate，都会被 Ribbon 拦截
3. **创建负载均衡器：**
    1. 拦截后，Ribbon 会创建一个**负载均衡器 `ILoadBalancer`**
    2. 这个负载均衡器**会自动初始化要使用的负载均衡算法、心跳检测，以及加载服务列表**
4. **负载均衡和路由转发**
    1. 负载均衡器会将 RestTemplate 中的服务名解析为具体的 IP
    2. 如果一个服务名有对应的多个 IP，则会使用负载均衡算法做负载均衡
    3. 确定好 IP 和 PORT 后，再从 feign 的代理对象中拿到 目标接口的 URL（[[OpenFeign 原理 ⭐]]），就会进行 HTTP 调用

    ![[IMG-20260405035413898.png|800]]
