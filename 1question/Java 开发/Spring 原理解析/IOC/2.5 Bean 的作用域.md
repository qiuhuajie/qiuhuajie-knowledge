...
2. Bean 的作用域是指 **Bean 在 Spring 整个框架中的某种行为模式**
3. 比如 singleton 单例作用域，（默认作用域为单例）就表示 Bean 在整个 Spring 中只有一份，它是全局共享的，当有人修改了这个值之后，那么另一个人读取到的就是被修改后的值
...
> 💡 ⚠️ 注意：后 3 种作用域，只适用于 Spring MVC 框架
...
2. 描述：
    ...
    2. 获取 Bean（即通过 applicationContext.getBean等方法获取）及装配 Bean（即通过 @Autowired 注入）都是同一个对象
    3. 加载 spring 配置文件时候就会创建单实例对象
3. 场景：
    1. **通常无状态的 Bean 使用该作用域**
    2. 无状态表示 Bean 对象的属性状态不需要更新
...
2. 描述：
    ...
    2. 获取 Bean（即通过 applicationContext.getBean 等方法获取）及装配 Bean（即通过 @Autowired 注入）都是新的对象实例
    3. 不是在加载 spring 配置文件时候创建对象，而是在调用 getBean 方法时候创建多实例对象
...
1. 可以通过 `@Scope`  注解来设置 Bean 的作用域，它的设置方式有以下两种：
    ...
    2. 方式二：设置 ConfigurableBeanFactory 和 WebApplicationContext 提供的 `SCOPE_XXX` 变量，如`@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)`
2. 具体设置代码如下：

    ![640 (1).png](IMG-20260620230826333.png)

    ![640 (2).png](IMG-20260620230826440.png)

    ![640 (3).png](IMG-20260620230826772.png)