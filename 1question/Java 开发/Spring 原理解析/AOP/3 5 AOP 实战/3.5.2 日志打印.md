...
2. 流水的写入对一致性的要求不是很高，所以没必要去保证事务
3. 希望对主链路的侵入越少越好
4. 解决方案：在每一个操作的方法之前，去加一个注解，通过切面处理这个方法，在处理逻辑中异步地去写入流水表
5. **⭐难点：**
    1. **虽然写入流水是一个标准化的模型，但是入参并不标准**
    2. **例如入参是不同的类，但是不同的类里都有商家ID，但是有的类中定义为 `orderId`，有的类中定义为 `id`**
    ...
    3. **那么如何在统一的一个切面中，去根据不同的入参，取到想要的参数值**
    4. 难道要在切面的处理逻辑中逐个判断入参的类型，然后进行转换吗，显然不太优雅
...
2. `SaveOrderConvert` ：**在这个实现类中，`orderId` 取得是 `id` 字段**
...
3. `UpdateOrderConvert` ：**在这个实现类中，`orderId` 取得是 `orderId` 字段**
...
![Untitled](IMG-20260620224657977.png)

![Untitled](IMG-20260620230831671.png)
