1. **`Optional<T>`** 类(java.util.Optional) 是一个==**容器类**==
    ...
    1. 或者仅仅保存 null，表示这个值不存在
    
1. 原来用 null 表示一个值不存在，现在 Optional 可以更好的表达这个概念。并且**可以避免空指针异常**
1. Optional 提供的方法
    ...
    1. 创建Optional类对象的方法：
        ...
        1. `Optional.ofNullable(T t)`： **t 可以为null**
    ...
    1. 判断 Optional 容器中是否包含对象：
        ...
        1. `void ifPresent(Consumer<? super T> consumer)` ： 如果有值，就执行Consumer接口的实现代码，并且该值会作为参数传给它
    ...
    1. 获取 Optional 容器的对象：
        ...
        1. `T orElseGet(Supplier<? extends T> other)` ： 如果有值则将其返回，否则返回由Supplier接口实现提供的对象
        
        1. `T orElseThrow(Supplier<? extends X> exceptionSupplier)` ： 如果有值则将其返回，否则抛出由Supplier接口实现提供的异常
