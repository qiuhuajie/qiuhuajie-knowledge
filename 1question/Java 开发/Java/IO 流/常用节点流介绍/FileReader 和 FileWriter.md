...
1. **`FileReader`** 字符流**支持每次读取文件的时候按照** **`char`** **为基本单位进行读取**
    ...
    2. 同样的，==`FileReader`====的====`read`====函数在调用完之后会返回读取到的文本内容长度，如果长度为====`-1`====，则表示读取到的内容已经结束==
2. **`FileWriter`**是一款基于字符为基本单位的输出流，可以用于往文本文件内部写入一定的数据
# 二、代码示例
## 1. FileReader
```Java
