...
2. Java IO 原理：
    ...
    > [!important] 一个汉字占两个字节长度（因为汉字博大精深，所以有些汉字也会占到
    >
    > **「三个字节」**的长度）
# 2. IO 流的分类
## 2.1 按**操作数据单位**分
1. **字节流**
    ...
    2. 例如读取数据的时候每次如同下图所示，每次都按照字节**（`8bit-byte`）**进行加载
        ...
        ![[Attachment/1question/大数据/Java 开发/Java/IO 流/IMG-20260405035413788.png|Untitled 282.png]]
...
2. **字符流**
    ...
    2. 例如读取数据的时候每次如同下图所示，每次都按照两个字节**（`16bit-char`）**进行加载
        ...
        ![[IMG-20260404031802212.png|Untitled 1 211.png]]
...
> [!important] **输入是指从外界输入到内存，输出是指从内存输出到外界**
## 2.3 按**流的角色**的不同分为：
1. 节点流
...
| **抽象接口（基类）** | InputStream          | OutputStream          | Reader            | Writer             | **节** **点** **流** |
...
- **点击查看完整 Java 的 IO 类结构图**
    ...
    [![](http://web.deu.edu.tr/doc/oreily/java/fclass/figs/jfc_1101.gif)](http://web.deu.edu.tr/doc/oreily/java/fclass/figs/jfc_1101.gif)
