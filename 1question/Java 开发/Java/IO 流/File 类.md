...
3. 解决方法：
    ...
    2. **`File.separator` 会根据操作系统，动态的提供分隔符**
...
> 2. **后续** **`File`** **类的对象会作为参数传递到**==**流的构造器**==**中，指明读取或写入的”终点“**
## 1. File类的获取功能
...
7. `String[] list()`： 获取指定目录下的所有文件或者文件目录的**名称数组**
    ...
    ```
8. `File[] listFiles()`： 获取指定目录下的所有文件或者文件目录的**File数组**
...
3. `boolean mkdirs()` ： 创建文件目录。 如果**输入路径中上层文件目录不存在， 一并创建**
## 5. File类的删除功能
