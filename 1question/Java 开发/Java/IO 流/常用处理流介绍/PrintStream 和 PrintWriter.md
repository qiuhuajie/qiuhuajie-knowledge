...
1. 提供了**一系列重载的**`**print()**`**和**`**println()**`**方法**，用于多种数据类型的输出
1. `PrintStream`和`PrintWriter`的输出不会抛出`IOException`异常
1. `PrintStream`和`PrintWriter`有自动`flush`功能
1. `PrintStream`打印的所有字符都使用平台的默认字符编码转换为字节；因此在需要写入字符而不是写入字节的情况下，应该使用 `PrintWriter`类
1. 其实在日常开发中经常会接触到`PrintStream`这款标准输出字节流，例如刚入门阶段所接触的`helloworld`案例，底层就是基于`PrintStream`去做的
...
![[IMG-20260404031802407.png|Untitled 455.png]]
## 2.2 **PrintWriter**
```Java
public static void main(String[] args) throws IOException {
