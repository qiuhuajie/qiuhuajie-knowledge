...
- [[#== 、 equals() 和 hashCode()🙋‍♂️]]
    ...
    - [[#equals()]]
    - [[#hashCode() ]]
    - [[#重写 equals 时为什么一定要重写 hashCode🙋‍♂️]]
...
1. 其核心之处就在于
    ...
    1. 虚拟机会在执行程序时动态调用实际类的方法，它会通过一种名为**动态绑定机制**自动实现，这个过程对程序员来说是透明的
...
1. 举例
    ...
    1. 在定义方法参数时也通常总是应该优先使用父类类型或接口类型，例如某方法应该写成：
    ...
    1. 这样声明最大的好处在于结构的==**灵活性：**==
        
        1. 假如某一天我认为 ArrayList 的特性无法满足我的要求，我希望能够用 LinkedList 来代替它
        
        1. 那么只需要在对象创建的地方把 `new ArrayList()` 改为 `new LinkedList` 即可，其它代码一概不用改动
...
1. **重写（Override）**
    ...
    1. 重写的异常抛出限制：
        ...
        1. 如果父类抛出的是非运行时异常（IOException、CharacterCodingException），重写的方法抛出的异常必须小于等于被重写方法抛出的异常
...
1. **重载（Overload）**
    ...
    1. 存在于同一个类中，指一个方法与已经存在的方法名称上相同，但是参数的类型、个数、顺序至少有一个不同
    
    1. 应该注意的是，返回值不同，其它都相同不算是重载
...
1. 是 Java 中 **Object 类的一个方法，用于比较一个对象是否等于另外一个对象**
    ...
    1. 但往往更希望比较的是对象的内容，因此**可以根据需要在子类中重写** `**equals()**` **方法来**==**比较对象的**====**值**====**是否相等**==
    
    1. 例如，String 就重写了 equals() 方法
...
1. **作用：**hashCode 方法在配合使用散列集合时，==判断元素是否存在时会使用到，可以提高在判断元素是否相同时，降低==调用 `equals()` 的频率，提升性能
    
    1. 如 HashMap、HashSet，因为这些集合的 `put()` 中会需要判断元素是否存在，如果只有 `equals()` 方法的话，需要一个个比较，效率太低
    
    1. 但是如果使用 `hashCode` 的话，经过散列函数（取模）就可以得到元素该存的索引下标
        
        1. 如果得到的索引值没有元素，则直接将当前元素直接放入集合
        
        1. 如果存在元素，才会进一步调用 `equals()` 比较两个元素的值是否相等
            
            ![[IMG-20260405035425820.png|Untitled 3 26.png]]
            
        
    
1. 因此，降低了调用 `equals()` 的频率
### **重写 equals 时为什么一定要重写 hashCode**🙋‍♂️
1. ==hashCode 的值默认是 JVM 使用随机数来生成的==
1. ==两个不同的对象生成的 hashCode 可能会相同==，这种情况，一般使用拉链法来解决这类哈希冲突问题
1. 但还会出现==两个相同的对象生成的 hashCode 可能会不同==，那么该类对象在配合使用散列集合时，就会有问题
    
    1. 例如：`a.equals(b) == ture`
    
    1. 如果没有重写 equals 方法，默认 equals 比较内存指向，因此这两个对象的内存地址一定是同一个，必然相等
    
    1. ⭐如果只重写 equals 方法，但没有重写 hashCode 方法，此时这两个对象的地址指向可能不同，但内容相同；==但由于 hashCode 是随机生成的，可能会不同，因此在配合散列集合使用的时候，会直接取模到不同的位置，==**==造成两个相同的对象，插入散列集合的不同位置==**
        
        ![[IMG-20260405035439065.png|Untitled 4 22.png]]
        
    
    1. 因此，约定要重写 `hashCode()` 来保证，相同对象的 hashCode 一定相同！
    
1. **如何重写❓**
    
    1. 理想的散列函数应当具有均匀性，即不相等的对象应当均匀分布到所有可能的散列值上
    
    1. 可以将**每个域（对象属性）都当成 R 进制的某一位**，然后组成一个 R 进制的整数
    
    1. **R 一般取** `**31**`，因为它是一个奇素数，如果是偶数的话，当出现乘法溢出，信息就会丢失，因为与 2 相乘相当于向左移一位
...
1. **`String` 有**==**不可变**==**的特性**
    ...
    1. 内部使用 `char` 数组存储数据，该数组被声明为 `final`，因此 **value 数组初始化之后就不能再引用其它数组**，并且 String 内部没有改变 value 数组的方法，因此可以保证 String 不可变
        
        ![[IMG-20260405035501337.png|Untitled 5 22.png]]
        
    
    1. ==这意味着==**==每次对 String 对象进行操作时，都需要创建一个新的 String 对象==**==，因为原来的 String 对象不能更改==
...
1. **不可变的好处：**
    ...
    1. 只有不可变，才支持：如果一个 String 对象已经被创建过了，那么就会从 **String Pool** 中取得引用
    
1. **坏处：**这样会导致大量的内存分配和垃圾回收操作，特别是在处理大量字符串时会对性能产生负面影响
## StringBuilder
1. **`StringBuilder`** 是一个==**可变**==**的对象，它允许对字符串进行修改而不创建新的对象**
    
    1. 继承自 `AbstractStringBuilder` ，使用一个非 `final` 的 `char[]`
        
        ![[IMG-20260405035509708.png|Untitled 6 19.png]]
        
    
    1. 它通过**预先分配一段足够大的内存空间**来避免频繁的内存分配和垃圾回收操作
    
    1. 因此，当需要对字符串进行==**频繁的修改**==**时，应该使用 StringBuilder**；当需要处理不可变字符串时，应该使用String
...
1. StringBuffer 还有一个重要的特性是==**线程安全**==
    
    1. 方法实现中使用 **`_synchronized_`** 关键字来保证线程安全，在多线程环境下使用它可以避免线程冲突的问题
        
        ![[IMG-20260405035510554.png|Untitled 7 18.png]]
        
    
    1. 故性能略低于 StringBuilder
...
1. 什么是**泛型的类型擦除❓**
    ...
    1. 虽然 `ArrayList<String>` 和 `ArrayList<Integer>` 在编译时是不同的类型，但是在编译完成后都被编译器简化成了`ArrayList`，这一现象，被称为泛型的**类型擦除**(Type Erasure)
    
1. **为什么要进行泛型的类型擦除❓**
    
    1. 主要目的是避免过多的创建类而造成的运行时的过度消耗
    
    1. 如果用`List<A>`表示一个类型，再用`List<B>`表示另一个类型，以此类推，无疑会引起类型的数量爆炸
...
1. **反射能获取泛型的类型吗**❓
    ...
    1. 可以看到通过`getTypeParameters`方法==只能获取到泛型的参数占位符==，而不能获得代码中真正的泛型类型
    
1. **能在指定类型的List中放入其他类型的对象吗**❓
    
    1. 可以通过反射，绕过编译检查，把一个类型的对象能放进另一类型的 `List`
        
        ![[IMG-20260405035517681.png|Untitled 8 15.png]]
        
    
    1. 不仅在编译期间可以通过语法检查，并且也可以正常地运行，使用`debug`来看一下数组中的内容
        
        ![[IMG-20260405035525248.png|Untitled 9 12.png]]
        
    
    1. 但是，打印会报错，因为`ArrayList`中`get`方法的源码中会进行强制类型转换
        
        ![[IMG-20260405035530865.png|Untitled 10 12.png]]
        
        ![[IMG-20260405035534103.png|Untitled 11 12.png]]
...
1. **浅拷贝**
    ...
    - ==如果原对象中的属性是引用类型，那么浅拷贝得到的对象和原对象会共享同一个属性对象==
        ...
        ![[IMG-20260405035537200.png|Untitled 12 10.png]]
...
1. **深拷贝**
    ...
    - 深拷贝会复制对象本身以及对象所包含的所有属性和子属性
        ...
        ![[IMG-20260405035537771.png|Untitled 13 10.png]]
...
1. **成员变量**
    ...
    1. 可以被 `public` ，`private` ，`static` 等修饰符所修饰
    
    1. 静态成员变量存在方法区，非静态成员变量存在堆中
    
    1. 生命周期：是对象的一部分，它随着对象的创建而存在
    
1. **局部变量**
    
    1. 是在**方法中**定义的变量或是方法的参数
    
    1. 局部变量不能被访问控制修饰符及 `static` 所修饰
    
    1. 如果局部变量类型为基本数据类型，那么存储在栈内存，如果为引用数据类型，那存放的是指向堆内存对象的引用或者是指向常量池中的地址
    
    1. 生命周期：局部变量随着方法的调用而自动消
...
1. **基本类型、包装类与String类间的转换**
    ...
    ![[IMG-20260405035543272.png|Untitled 14 10.png]]
    
1. 基本数据类型的自动拆箱可能会有空指针异常
    
    ![[IMG-20260405035546497.png|Untitled 15 10.png]]
    ...
    1. **如果包装类 Boolean 对象是一个空对象 `NULL` 时，那么在调用 `booleanValue()`方法时便会抛出 `NullPointerException`**
...
1. **普通代码块**
    ...
    1. 普通代码块就是定义在方法中的代码块，也叫本地代码块
        
        ![[IMG-20260405035549054.png|Untitled 16 6.png]]
...
1. **实例代码块**
    ...
    1. 作用：一般**==用于初始化实例成员变量==**
        ...
        ![[IMG-20260405035549462.png|Untitled 17 6.png]]
        
    
    1. 实例代码块内部也可以初始化静态成员变量，但一般不这么做
...
1. **静态代码块**
    ...
    1. 作用：一般**==用于初始化静态成员变量==**
        ...
        ![[IMG-20260405035551966.png|Untitled 18 5.png]]
...
1. 代码的执行顺序为：
    ...
    1. 如果有多个实例代码块，则根据它们的定义顺序来先后执行
    
    1. 如果有多个静态代码块，在编译代码时，编译器会按照定义的顺序依次合并
...
1. **概述**
    ...
    1. 集合对象每次调用`**iterator()**`方法都得到一个**全新的迭代器对象**，默认游标都在集合的**第一个元素之前**
...
1. **Iterator 接口的使用**
    ...
    1. `**hasNext()**`、`**next()**`
        ...
        1. ⭐==一个====`while (iterator.hasNext())`== ==中不能出现两个== ==`.next()`====，否则会报====`NoSuchElementException`====异常（线程越界）==
...
1. 介绍
    ...
    1. Java实现**对象排序**的方式有两种：
        
        1. 自然排序：`**java.lang.Comparable**`，一旦指定一劳永逸
        
        1. 定制排序：`**java.util.Comparator**`，临时性的给定比较规则
    ...
    1. ⭐==**两者的使用场景不同：**==
        ...
        1. 使用 Comparable 必须==**要修改原有的类**==，也就是你要排序那个类
            ...
            1. 所以 Comparable 更像是**“对内”进行排序的接口**
            
        
        1. 而 Comparator 的使用则不相同，Comparator ==**无需修改原有类**==
            
            1. 也就是在最极端情况下，即使 Person 类是第三方提供的，我们依然可以通过创建新的自定义比较器 Comparator，来**实现对第三方类 Person 的排序功能**
            
            1. 也就是说通过 Comparator 接口可以实现和原有类的解耦，在不修改原有类的情况下实现排序功能
            
            1. 所以 Comparator 可以看作是**“对外”提供排序的接口**
...
1. **自然排序：java.lang.Comparable**
    ...
    1. **==重写====`compareTo()`====方法的规则：==**
        
        1. 如果当前对象this大于形参对象obj， 则返回正整数
        
        1. 如果当前对象this小于形参对象obj， 则返回负整数
        
        1. 如果当前对象this等于形参对象obj， 则返回零
        
    
    1. **对于自定义类，**==**如果需要排序，可以让自定义类实现 Comparable 接口，重写compareTo()方法**==
...
1. **定制排序：java.util.Comparator**
    ...
    1. 使用场景
        ...
        1. 或者**实现了 java.lang.Comparable 接口的排序规则，但是此规则不适合当前的操作**
        
    
    1. 那么**==可以考虑使用== ==`Comparator`== ==的对象来排序==**
    
    1. **==重写====`compare(Object o1,Object o2)`====方法的规则：==**
