...
> 💡 **在Git 2.23版本以后，增加了`git switch`、`git reset` 指令**
>
...
5. 开发案例：刚刚的更改 `commit` 错分支了，想撤销最近一次提交，但保留代码改动
    1. 这种场景适合使用：
    ...
    2. 这条命令的含义：
        - `git reset`：回退提交记录
        - `--soft`：只撤销 `commit`，代码改动保留在暂存区（staged）
        - `HEAD~1`：回退最近 1 个提交
    3. 为什么这里选 `--soft`
        - 因为目标只是把提交从当前分支上拿掉，并不是丢弃代码
        - 回退后改动还在暂存区，切到正确分支后可以直接重新提交
    4. 三种模式在这个场景下的区别：
...
| `--hard` | 撤销 | 撤销 | 撤销 |
    ...
    6. 一般处理流程如下：
        ...
        $ git reset --soft HEAD~1
        $ git switch 正确分支
        $ git commit -m "重新提交到正确分支"
        ```
    > ⚠️
    > 这个做法更适合“最近一次提交还没有 push 到远程”的情况。如果已经推送到远程并且别人可能基于这个提交继续开发，通常应优先考虑 `git revert`，或者在确认影响范围后再决定是否强推。
