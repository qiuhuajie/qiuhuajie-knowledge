...
1. **单节点 Nginx 存在的问题**
    ...
    2. 问题 2：nginx 存在单点故障，当唯一的 nginx 服务器宕机后，对外界则不能提供服务
2. 解决：
...
3. **如何实现❓**
    1. 设想的解决办法：
        ...
        2. **简单的方法就是直接将 192 的 IP 修改为192.168.10.192，但是这样存在很大问题** 🚫
            1. 如果突然 191 又恢复了，此时同一个局域网中存在两个相同的 IP ，显然是不可以的
            2. 故不能简单的使用 IP 复制的方法来解决 Nginx 高可用的场景
    2. **解决方案** ✅
        - ==**配合 keepalived 软件 + VIP（虚拟 IP）漂移**==

    ![[IMG-20260620224140007.png|800]]
# 二、示例
1. 新建一个 nginx 的备用机 192.168.10.190 nginx_backup
2. 191 和 190 都安装 ==**keepalived**==
...
4. 修改 190 的 keepalived 配置文件
    ...
    ```
5. 各自启动 keepalived 服务
6. 此时观察
    1. 虚拟 IP 的位置
    2. 由于191 当前为 master，故虚拟 IP 在 191 上

    ![[IMG-20260620224145786.png|800]]
    ![[IMG-20260620224156031.png|800]]
7. 测试：
    1. 访问 [http://192.168.10.200/](http://192.168.10.200/)
    2. 成功访问
8. ==**测试两个主机对 虚拟 IP 的竞争过程**==
    ...
    来自 192.168.10.200 的回复: 字节=32 时间<1ms TTL=64
     ...
     来自 192.168.10.200 的回复: 字节=32 时间<1ms TTL=64
    ```
    4. 现在再看 虚拟 IP 的位置：==**已经挪到了备选机 190 上**==

    ![[IMG-20260620224203181.png|800]]
