...
2. 最简单的消息队列模型包括3个角色：
    ...
    ![[IMG-20260620224127758.png|800]]
3. **Redis 提供了三种不同的方式来实现消息队列：**
...
2. 队列是入口和出口不在一边，因此我们可以利用：`LPUSH` 结合 `RPOP`、或者 `RPUSH` 结合 `LPOP` 来实现
    > 注意：
    >
    > 1. 当队列中没有消息时RPOP或LPOP操作会返回null，并不像JVM的阻塞队列那样会阻塞并等待消息
    >
    > 2. 因此这里如果要实现阻塞式队列，在取出消息时应该使用`BRPOP`或者`BLPOP`

    ![[IMG-20260620224133713.png|800]]
3. 优点：
...
2. 消费者可以订阅一个或多个channel，生产者向对应channel发送消息后，所有订阅者都能收到相关消息
3. 相关命令：
    1. `SUBSCRIBE channel [channel]` ：订阅一个或多个频道
    2. `PUBLISH channel msg` ：向一个频道发送消息
    3. `PSUBSCRIBE pattern[pattern]` ：订阅与pattern格式匹配的所有频道（通配符匹配）

    ![[IMG-20260620224139311.png|800]]
4. 优点：
    1. 采用发布订阅模型
    2. 支持多生产、多消费
5. ==缺点：==
...
3. ==**读取消息：**====**`XREAD`**== ==**单消费者模式**==
    ...
    - 注意：`XREAD` **存在漏读消息**的问题：
        ...
        2. 如果**在我们处理一条消息的过程中，又有超过1条以上的消息到达队列，则下次获取时也只能获取到最新的一条**
        3. 会出现漏读消息的问题
...
4. ==**读取消息：**====**`XREADGROUP`**== ==**消费者组模式**== ==✔==
    ...
    2. 消费者组具备的特点
        - **消息分流：**
            ...
            2. 每个消息被同一组内的消费者竞争得到后，只会被消费一次，而不是重复消费，从而加快消息处理的速度
        - **消息标识：**
            1. 消费者组会维护一个标示，记录最后一个被处理的消息（书签，看到哪里了）
            2. 哪怕消费者宕机重启，还会从标示之后读取消息。确保每一个消息都会被消费，这样可以解决消息漏读的问题
        - **消息确认：**
            1. 消费者获取消息后，消息处于`pending` 状态，并存入一个`pending-list`
            2. 当处理完成后需要通过`XACK`来确认消息，标记消息为已处理，才会从pending-list移除
            3. 解决消息丢失问题：拿到了没消费，却把消息pop删除了的情况
    3. 操作消费组的命令：
        1. 创建消费组：

    ![[IMG-20260620224202464.png|800]]
            - `key`：队列名称
            - `groupName`：消费者组名称
...
```
    ...
    ![[IMG-20260620224213557.png|800]]
5. redis 的三种消息队列的比较

    ![[IMG-20260620224216644.png|800]]
