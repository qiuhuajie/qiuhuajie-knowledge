---
title: "基于 Redis 实现共享 session 登录"
tags:
  - "中间件"
  - "中间件/Redis"
  - "中间件/Redis/Redis"
...
# 一、业务流程
![[IMG-20260620224127737.png|800]]
![[IMG-20260620224131003.png|800]]
1. 注入redis依赖
...
2. 在保存用户信息时
    ...
    2. 单系统中
        ...
        2. **浏览器维护 cookie，请求时携带 cookie，而 cookie 中存着 sessionID（**服务器向用户浏览器发送了一个名为JESSIONID的Cookie，它的值是Session的id值）
        3. 因此，如果想要获取 session 中的信息直接：
    ...
    3. **但是，在多系统中**
        ...
        2. 故需要在创建用户时生成一个token，==token 的作用就是单系统中的== `cookie.JSESSIONID`
        3. 前端代码中获取到 token 在 `axios` 拦截器拦截到请求后，会**将 token 放在请求头的** **`authorization`** **字段中**（项目代码已经实现）
    ...
    ```
    4. 此时要将 user 对象存入 Redis，故 ==value 需要====**选用Hash类型**==
        1. 且一个对象要存储多个 hash 键值，就要用到 `stringRedisTemplate` 提供的`putall()`方法，但此方法需要传入一个HashMap
        2. 因此还需要先使用hutool工具提供的beanToMap将user对象转成HashMap
    5. 同时**==对存入 Redis 的对象设置有效期==（**==**session默认的有效期是30分钟**==**，使用 redis 代替session的话，也将其有效期设置成30分钟）**
3. 完整代码：
...
1. 自定义的 LoginInterceptor 登陆拦截器
    ...
    2. token 判空
        ...
        2. **放在网关层可以做的复杂一点，可以**==**对 Token 的合法性进行校验**==[[Gateway 项目实战]]
    3. **根据 token** ==**去 Redis 中查有没有对应的用户存在**==
    4. 并将查出的用户信息存在 ThreadLocal 中

    ![[IMG-20260620224131122.png|800]]
2. 最后，session 在每次有新的请求时，就会刷新有效期，所以基于Redis实现时，**==每次拦截到一个请求，也要将token的有效期刷新==**
3. 完整代码：
