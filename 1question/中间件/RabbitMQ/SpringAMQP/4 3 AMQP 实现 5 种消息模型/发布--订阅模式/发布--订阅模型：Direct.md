...
1. 使用场景：
    ...
    1. 但是，在某些场景下，我们**希望**==**不同的消息被不同的队列消费**==
    
    1. 这时就要用到`Direct`类型的Exchange
    
1. `Direct Exchange` 会将接收到的消息根据规则路由到指定的`Queue`，因此称为路由模式（`routes`）
1. 路由模式实现流程：
    
    ![[IMG-20260404031928043.png]]
    ...
    1. 消息的发送方**在向 `Exchange`发送消息时：**
        ...
        1. `Exchange`不再把消息交给每一个绑定的队列，而是**根据消息的`Routing Key`进行判断，只有队列的`Routingkey`与消息的 `Routing key`完全一致，才会接收到消息**
...
1. consumer 服务控制台：
    ...
    1. 当`rabbitTemplate.convertAndSend();`传入的路由键是 `red` 时，队列 1、2都可以接收到消息
        ...
        ![[IMG-20260404031928128.png]]
        
    
    1. 如果将消息的路由键改成 `blue`时，只有绑定了路由键`blue`的队列1 才能收到消息
        
        ![[IMG-20260404031928242.png]]