...
1. 下载安装anaconda
    ...
    2. 选择版本：[Anaconda3-5.2.0-Windows-x86_64.exe](https://repo.anaconda.com/archive/Anaconda3-5.2.0-Windows-x86_64.exe)
2. 创建虚拟环境
...
3. 下载安装pytorch
    ...
    2. 查看GPU型号是否支持cunda：[https://www.nvidia.cn/geforce/technologies/cuda/supported-gpus/?field_gpu_type_value=All](https://www.nvidia.cn/geforce/technologies/cuda/supported-gpus/?field_gpu_type_value=All)
    3. 查看DPU驱动版本与支持的cuda版本：
...
4. 确认安装成功
    ...
     >>>
    ```
5. 确认cuda正常使用：保证可以使用GPU来加速模型的训练
...
2. add_scalar 方法：
    ...
    2. 示例：
        ...
        ```
    3. 会在logs 文件夹下生成日志文件：
    4. 在 Terminal 中 使用 tensorboard 查看 logs 中的日志文件
...
3. add_image 方法：
    ...
    4. 或不使用 Image 读取数据，而使用 OpenCV
        ...
        ```
    5. 使用 tensorboard 查看 logs 中的日志文件
...
4. 常见的 Transform 的使用
    1. `ToTensor()`：Convert a PIL Image or numpy.ndarray to tensor
        ...
         print(image_tensor)
        ```
    2. `Normalize()`：Normalize a tensor image with mean and standard deviation
        ...
        write.close()
        ```
    3. `resize()`：Resize the input image to the given size
        ...
        write.close()
        ```
    4. `compose()`：Composes several transforms together
        ...
        write.close()
        ```
    5. `randomcrop()`：Crop the given image at a random location
...
1. 什么是卷积
    ...
    2. 不额外设置填充 padding = 0，stride = 2
    3. 额外设置填充 padding = 1（填充值为0），stride = 1
2. 二维卷积 torch.nn.Conv2d
    1. 参数
        ...
        2. out_channels：可以设置多个卷积核，这样就会输出多个结果
        3. kernel_size：卷积核的大小
        4. stride：移动步数
        5. padding：是否填充
    2. 输入输出的形状要求：
        1. input（bachsize,输入通道,输入高度,输入宽度）
        2. output（bachsize,输入通道,输入高度,输入宽度）
    3. 示例：
...
3. torch.nn.MaxPool2d
    ...
    2. **stride** – the stride of the window. Default value is `kernel_size`（默认步长是池化核的大小）
    ...
    5. **return_indices** – if `True`, will return the max indices along with the outputs. Useful for [`torch.nn.MaxUnpool2d`](https://pytorch.org/docs/stable/generated/torch.nn.MaxUnpool2d.html#torch.nn.MaxUnpool2d) later
...
1. ReLU
    ...
    2. Shape:
        ...
        2. Output: (*)(∗), same shape as the input.
    3. 示例：
...
2. 参数
    ...
    2. **out_features** – size of each output sample （输出规模）
    3. **bias** – If set to `False`, the layer will not learn an additive bias. Default: `True` （偏置）
3. 示例
...
> 1. 计算实际输出与目标之间的差距
> 
> 2. 为我们更新输出提供一定的依据（反向传播），利用梯度优化权重
1. L1LOSS
...
3. CrossEntropyLoss（交叉熵）
    ...
    2. 示例：
        1. 输入：dog的图像
        2. 分类：person, dog, cat { 0 1 2 }
        3. 输出：x=[0.1,0.2,0.3]
        4. target：class=1
        5. loss(x,class)= -0.2 +log(exp(0.1)+exp(0.2)+exp(0.3))
...
2. 要使用torch.optim，必须构造一个优化器对象，该对象将保存当前状态并根据计算出的梯度更新参数
3. 构建一个优化器：
...
5. 采取优化的步骤
    1. 大多数优化器支持的简化版本
        ...
        for input, target in dataset:
            ...
            optimizer.step()
        ```
    2. 某些优化算法（例如共轭梯度和 LBFGS）需要多次重新评估函数，因此您必须传入一个闭包，以便它们重新计算模型。闭包应该清除梯度，计算损失并返回它
...
2. **（推荐）**保存与加载的方式二：保存 模型的 参数（字典）
    ...
    # )
    ```
3. 如果是保存加载自己写的模型需要将模型的那个文件的所有模块都引入
