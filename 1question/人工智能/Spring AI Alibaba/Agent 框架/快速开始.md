...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 1

1. 跟随快速开始，学习如何开发一个具备完整功能的 ReactAgent 智能体。
...
1. 在使用之前，需要配置你的 API Key。推荐通过环境变量设置：

```bash
export AI_DASHSCOPE_API_KEY=your_api_key_here
```

2. 你也可以在应用配置文件中设置（不推荐在生产环境中硬编码）：
...
1. 首先创建一个简单的 agent，它可以回答问题并调用工具。该 agent 将使用 DashScope 大模型、一个天气查询工具、以及简单的系统提示词。
...
1. 接下来，构建一个实用的天气预报 agent，演示关键的生产概念：
...
1. 系统提示定义了 agent 的角色和行为。保持具体和可操作：
...
1. 工具让模型能够通过调用你定义的函数与外部系统交互。工具可以依赖运行时上下文，也可以与 agent 的记忆交互。
2. 注意下面的 `getUserLocation` 工具如何使用运行时上下文（通过 `ToolContext` ）：
...
3. **提示**: 工具应该有良好的文档：它们的名称、描述和参数名称都会成为模型提示的一部分。Spring AI 的 `FunctionToolCallback` 支持通过 `@ToolParam` 注解添加元数据，并支持通过 `ToolContext` 参数进行运行时注入。
...
1. 如果你需要 agent 响应匹配特定的模式，可以定义结构化响应格式。
...
1. 为你的 agent 添加记忆以维持跨交互的状态。这允许 agent 记住之前的对话和上下文，在多次调用之间，使用同一个 `threadId` 即可加载之前的对话记录。
...
3. **注意**: 在生产环境中，使用持久化的 CheckPointer 将数据保存到数据库。更多详情参见 [内存管理](https://java2ai.com/docs/frameworks/agent-framework/tutorials/memory/) 文档。

### 3.6 创建和运行 Agent

1. 现在用所有组件组装你的 agent 并运行它！
...
1. 完整示例代码请查看仓库： [https://github.com/spring-ai-alibaba/examples](https://github.com/spring-ai-alibaba/examples)

## 5. 开启高级功能

### 5.1 使用 outputSchema 定义输出格式

1. 除了使用 `outputType` ，你还可以使用 `outputSchema` 来定义自定义的 JSON 格式：
...
### 5.2 使用 Invoke 方法获取完整状态

1. 如果需要访问完整的 agent 状态（不仅仅是最后的消息），可以使用 `invoke` 方法：
...
1. 为防止无限循环，可以使用 `ModelCallLimitHook` 来限制模型调用次数：
...
1. ReactAgent 支持通过 Hooks 扩展功能，例如人机协同、工具注入等：
...
1. 现在你已经创建了一个基础的 ReactAgent，你可以：
* 探索更多的工具集成
