...
source: "https://java2ai.com/docs/frameworks/agent-framework/advanced/human-in-the-loop"
author:
...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 11

1. 人工介入（HITL）Hook 允许你为 Agent 工具调用添加人工监督。当模型提出需要审查的操作时——例如写入文件或执行 SQL——Hook 可以暂停执行并等待人工决策。
2. 它通过检查每个工具调用并与可配置的策略进行比对来实现。如果需要人工干预，Hook 会发出中断（interrupt）来暂停执行。图的状态会通过 Spring AI Alibaba 的检查点机制保存，因此执行可以安全暂停并在之后恢复。
3. 人工决策决定接下来发生什么：操作可以被原样批准（ `approve` ）、修改后运行（ `edit` ）或拒绝并提供反馈（ `reject` ）。

## 1. 中断决策类型

1. Hook 定义了三种人工响应中断的内置方式：
...
2. 每个工具可用的决策类型取决于你在 `approvalOn` 中配置的策略。当多个工具调用同时暂停时，每个操作都需要单独的决策。

## 2. 配置中断

1. 要使用 HITL，在创建 Agent 时将 Hook 添加到 Agent 的 `hooks` 列表中。
2. 你可以配置哪些工具需要人工审批，以及为每个工具允许哪些决策类型。
...
1. 当你调用 Agent 时，它会一直运行直到完成或触发中断。当工具调用匹配你在 `approvalOn` 中配置的策略时会触发中断。在这种情况下，调用结果将返回 `InterruptionMetadata` ，其中包含需要审查的操作。你可以将这些操作呈现给审查者，并在提供决策后恢复执行。
...
1. Hook 定义了一个在模型生成响应后但在执行任何工具调用之前运行的 `afterModel` 钩子：
1. Agent 调用模型生成响应。
2. Hook 检查响应中的工具调用。
...
## 6. Workflow 中嵌套 Agent 的人工中断

1. 在复杂的应用场景中，你可能需要在 `StateGraph` 工作流中嵌套 `ReactAgent` ，并为嵌套的 Agent 配置人工介入能力。这允许你在工作流执行过程中对 Agent 的工具调用进行人工监督。

### 6.1 工作原理

1. 当 `ReactAgent` 作为工作流中的一个节点时，如果 Agent 配置了 `HumanInTheLoopHook` ，工作流会在 Agent 节点触发工具调用中断时暂停。工作流级别的中断处理与单独的 Agent 中断处理类似，但需要在 `CompiledGraph` 层面进行恢复。
...
1. 与单独使用 Agent 相比，在 Workflow 中使用人工中断有以下关键区别：
