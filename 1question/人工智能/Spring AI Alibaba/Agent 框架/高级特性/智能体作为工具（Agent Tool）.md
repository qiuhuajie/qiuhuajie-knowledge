...
source: "https://java2ai.com/docs/frameworks/agent-framework/advanced/agent-tool"
author:
...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 14
...
1. 在 **工具调用** 中，一个Agent（" **控制器** "）将其他Agent视为 *工具* （AgentTool），在需要时调用。控制器管理编排，而工具Agent执行特定任务并返回结果。
2. 流程：
...
![[Attachment/abfcc7f1795d7b965131c9759e04857b_MD5.png]]

> 作为工具使用的Agent通常 **不期望** 与用户继续对话。它们的角色是执行任务并将结果返回给控制器Agent。如果你需要子Agent能够与用户对话，请改用\*\*交接（HandOff）\*\*模式。

3. 关于工具调用模式的使用请查看 [HandOffs 模式文档](https://java2ai.com/docs/frameworks/agent-framework/advanced/multi-agent) 。

### 4.1 实现

1. 下面是一个最小示例，其中主Agent通过工具定义访问单个子Agent：
...
1. 你可以在几个点控制主Agent和子Agent之间的上下文传递：
...
1. 有两个主要杠杆来控制主Agent传递给子Agent的输入：
...
1. 使用标准的 JSON Schema 格式定义输入结构，确保子Agent能够接收结构化的输入信息：
...
1. 使用 Java 类型定义输入，框架会自动生成 JSON Schema：
...
1. 使用 Java 类型定义输出，框架会自动生成输出 schema：
...
1. 同时使用 `inputType` 和 `outputType` 进行完整的类型化Agent工具调用：
...
1. 在实际应用中，主Agent通常需要访问多个不同的子Agent工具，根据任务需求选择合适的工具进行调用。这种模式允许你构建更灵活、更强大的多Agent系统。
