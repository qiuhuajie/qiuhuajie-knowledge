...
source: "https://java2ai.com/docs/frameworks/agent-framework/advanced/memory"
author:
...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 12
## 1. 概述

1. Spring AI Alibaba 的 Agent 使用持久化机制来实现长期记忆功能。这是一个高级主题，需要对 Spring AI Alibaba 的 Agent 框架有一定了解。
...
1. Spring AI Alibaba 将长期记忆以 JSON 文档的形式存储在 Store 中。
2. 每个记忆都在自定义的 `namespace` （类似于文件夹）下组织，并使用唯一的 `key` （类似于文件名）。命名空间通常包含用户或组织 ID 或其他标签，以便更容易地组织信息。
3. 这种结构支持记忆的层次化组织。通过内容过滤器支持跨命名空间搜索。
...
1. 下面的示例展示了如何创建一个工具，让 Agent 能够查询用户信息。
...
## 5. 使用 ModelHook 管理长期记忆

1. 下面的示例展示了如何使用 ModelHook 在模型调用前后自动加载和保存长期记忆。
...
1. 短期记忆用于存储对话上下文，长期记忆用于存储持久化数据。下面的示例展示了如何同时使用两种记忆。
