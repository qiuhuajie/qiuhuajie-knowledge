...
source: "https://java2ai.com/docs/frameworks/agent-framework/tutorials/agents"
author:
...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 2

1. Agents 将大语言模型与工具结合，创建具备任务推理、工具使用决策、工具调用的自动化系统，系统具备持续推理、工具调用的循环迭代能力，直至问题解决。
2. Spring AI Alibaba 提供了基于 `ReactAgent` 的生产级 Agent 实现。
3. **一个 LLM Agent 在循环中通过运行工具来实现目标** 。Agent 会一直运行直到满足停止条件 —— 即当模型输出最终答案或达到迭代限制时。

## 1. ReactAgent 理论基础

### 1.1 什么是 ReAct

1. ReAct（Reasoning + Acting）是一种将推理和行动相结合的 Agent 范式。在这个范式中，Agent 会：
...
### 1.2 ReactAgent 的工作原理

1. Spring AI Alibaba 中的 `ReactAgent` 基于 **Graph 运行时** 构建。Graph 由节点（steps）和边（connections）组成，定义了 Agent 如何处理信息。Agent 在这个 Graph 中移动，执行如下节点：
* **Model Node (模型节点)** ：调用 LLM 进行推理和决策
* **Tool Node (工具节点)** ：执行工具调用
* **Hook Nodes (钩子节点)** ：在关键位置插入自定义逻辑
2. ReactAgent 的核心执行流程：
...
1. Model 是 Agent 的推理引擎。Spring AI Alibaba 支持多种配置方式。

#### 基础模型配置

1. 最直接的方式是使用 `ChatModel` 实例：
...
1. 工具赋予 Agent 执行操作的能力，支持顺序执行、并行调用、动态选择和错误处理。
...
1. **ReAct 循环示例** ：Agent 自动交替进行推理和工具调用，直到获得最终答案。
...
1. System Prompt 塑造 Agent 处理任务的方式。

#### 基础用法

1. 通过 `systemPrompt` 参数提供字符串：
...
1. 使用 `ModelInterceptor` 实现基于上下文的动态提示：
...
1. 在某些情况下，你可能希望 Agent 以特定格式返回输出。ReactAgent 提供了两种策略。

#### 使用 outputType

1. 通过 Java 类定义输出结构，Agent 会自动生成对应的 JSON Schema：
...
1. 使用 `BeanOutputConverter` 生成输出 Schema，提供类型安全和自动 schema 生成：
...
1. Agent 通过状态自动维护对话历史。使用 `MemorySaver` 配置持久化存储。
...
2. **生产环境** ：使用 `RedisSaver` 、 `MongoSaver` 等持久化存储替代 `MemorySaver` 。

### 4.3 Hooks（钩子）

1. Hooks 允许在 Agent 执行的关键点插入自定义逻辑。
...
### 4.4 Interceptors（拦截器）

1. Interceptors 提供更细粒度的控制，可以拦截和修改模型调用和工具执行。
...
1. 通过 Hooks 控制 Agent 的执行迭代，防止无限循环或过度成本。
...
1. 在 Agent 场景中，流式输出的核心是处理 `StreamingOutput` 类型。无论是模型推理、工具调用还是 Hook 节点，输出都统一为这个类型。
2. **使用 OutputType 区分输出类型** ：
3. 通过 `OutputType` 枚举可以区分不同节点的输出，以及判断是流式增量内容还是完成输出：
...
1. `StreamingOutput.message()` 返回的消息有多种类型，需要结合 `OutputType` 和消息元数据进行区分处理：
...
2. 更多关于 Graph 底层流式输出机制，请参考 [Graph 流式输出](https://java2ai.com/docs/frameworks/graph-core/core/streaming) 。
