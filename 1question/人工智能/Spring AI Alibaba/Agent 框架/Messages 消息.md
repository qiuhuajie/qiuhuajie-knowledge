...
source: "https://java2ai.com/docs/frameworks/agent-framework/tutorials/messages"
author:
...
tags:
  - "clippings"
---
# 一、Agentic AI Framework for Java Developers 4

1. Messages 是 Spring AI Alibaba 中模型交互的基本单元。它们代表模型的输入和输出，携带在与 LLM 交互时表示对话状态所需的内容和元数据。
2. Messages 是包含以下内容的对象：
* **Role（角色）** - 标识消息类型（如 `system` 、 `user` 、 `assistant` ）
* **Content（内容）** - 表示消息的实际内容（如文本、图像、音频、文档等）
* **Metadata（元数据）** - 可选字段，如响应信息、消息 ID 和 token 使用情况
3. Spring AI Alibaba 提供了一个标准的消息类型系统，可在所有模型提供商之间工作，确保无论调用哪个模型都具有一致的行为。

## 1. 基础使用

1. 使用 messages 最简单的方式是创建消息对象并在调用模型时传递它们。
...
1. 文本提示是字符串 - 适用于简单的生成任务，不需要保留对话历史。
...
1. `SystemMessage` 表示一组初始指令，用于引导模型的行为。你可以使用系统消息来设置语气、定义模型的角色并建立响应指南。
...
1. `UserMessage` 表示用户输入和交互。它们可以包含文本、图像、音频、文件和任何其他数量的多模态内容。
...
1. **注意** ：元数据字段的行为因提供商而异 - 有些用于用户识别，有些则忽略它。要检查，请参考模型提供商的文档。

#### 多模态内容

1. `UserMessage` 可以包含多模态内容，如图像：
...
1. `AssistantMessage` 表示模型调用的输出。它们可以包括多模态数据、工具调用以及你稍后可以访问的提供商特定元数据。
...
2. `AssistantMessage` 对象由模型调用返回，其中包含响应中的所有相关元数据。
3. 提供商对消息类型的权重/上下文化方式不同，这意味着有时手动创建新的 `AssistantMessage` 对象并将其插入消息历史中（就像它来自模型一样）会很有帮助。
...
1. 当模型进行工具调用时，它们包含在 `AssistantMessage` 中：
...
1. Spring AI Alibaba 的 `ChatResponse` 可以在其元数据中保存 token 计数和其他使用元数据：
...
### 2.4 Tool Response Message

1. 对于支持工具调用的模型，AI 消息可以包含工具调用。工具消息用于将单个工具执行的结果传回模型。
...
1. **多模态性** 指的是处理不同形式数据的能力，如文本、音频、图像和视频。Spring AI Alibaba 包含这些数据的标准类型，可以跨提供商使用。
2. 聊天模型可以接受多模态数据作为输入并生成它作为输出。下面我们展示包含多模态数据的输入消息的简短示例。
...
1. **警告** ：并非所有模型都支持所有文件类型。请查看模型提供商的文档以了解支持的格式和大小限制。

## 4. 与 Chat Models 一起使用

1. Chat models 接受消息对象序列作为输入并返回 `ChatResponse` （包含 `AssistantMessage` ）作为输出。交互通常是无状态的，因此简单的对话循环涉及使用不断增长的消息列表调用模型。
...
1. Spring AI Alibaba 的消息类提供了 builder 模式以便于构建：
...
1. ReactAgent 自动管理消息历史，但你也可以直接使用消息：
