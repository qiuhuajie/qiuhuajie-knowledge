---
title: "Claude Code 源码深度架构分析 - ATA"
tags:
  - "人工智能"
  - "人工智能/AI Coding"
...
> 发表：2026-03-31
> 更新：2026-04-08
> 链接：[原文](https://ata.atatech.org/articles/11020605711)
> 说明：本整理稿保留了原文的架构分析主线、关键图示和代码片段，移除了 ATA 页面抓取残留的作者勋章、分享按钮、朗读入口等网页噪声，并归档到 ClaudeCode 主题目录。
> [!abstract] 核心摘要
> 这篇文章围绕 Claude Code 源码拆出三条主线：其一是 QueryEngine 与 queryLoop 组成的 Agent Loop，负责会话管理、错误恢复、上下文压缩与流式工具执行；其二是 Tool 系统，通过严格的权限、并发与上下文约束把模型能力映射成可审计的环境操作；其三是 Prompt、Context 与 Harness 三层工程设计，展示一个成熟 Coding Agent 如何在大规模代码与长时任务中保持稳定性、安全性与可扩展性。
> 文章的价值不只是“读源码”，而是给出了一套分析 Agent 系统的方法：先看循环骨架，再看工具边界，最后再看上下文治理与运行时安全机制如何闭环。

# 一、我的整理
## 1. 一句话结论
* Claude Code 最值得学习的，不是某个孤立模块，而是它把 Agent Loop、Tool System、Context Engineering 和安全策略做成了一个相互配合的完整运行系统。

## 2. 全文主线
1. 先从 Agent Loop 入手，理解 QueryEngine、queryLoop 与 StreamingToolExecutor 如何驱动整套系统运行。
2. 再看 Tool System，理解 Claude Code 如何把模型能力严格约束在工具边界内，并配套权限、并发与安全防线。
3. 然后分析 Prompt / Context / Harness 三层设计，理解它如何在复杂任务中维持长上下文、稳定执行与可恢复性。
4. 最后回到方法论层面，总结 Claude Code 为什么能成为 AI Coding Agent 的工程标杆。

## 3. 最值得记住的判断
* Claude Code 的核心不是“会调工具”，而是它把工具、上下文、权限与恢复机制都纳入了统一执行框架。
* 它对安全与稳定性的追求非常工程化，很多默认值都体现了 fail-closed 思想。
* 真正成熟的 Agent，不是靠一段 Prompt 撑起来，而是靠循环控制、上下文治理和运行时约束共同支撑。
* 如果想做 AI Coding Agent，Claude Code 更像是一套系统设计样板，而不是几个技巧的集合。
...
# 三、正文整理
* 以下正文以原文内容和结构为主，仅做归档清理与排版整理。
> 基于 `@anthropic-ai/claude-code` v2.1.88 源码（51万行 TypeScript，1902 个文件）
> 
> 价值200🔪的分析报告🐶
> 
> ![image-20260331193654560](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961760551-daa96a00-2654-475c-912f-032d30055ea3.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
> 
...
![架构鸟瞰](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961320587-4c0a32dd-eea5-40b6-9853-3c65efc4bc56.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![两层循环模型](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961323476-a1f40b48-0260-4ec2-b759-16b0aeeac5fd.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![queryLoop 状态转换图](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961326983-65cb2dda-ec03-48f6-b617-8a1ffd8ebbb9.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![消息预处理管线](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961329917-ce5f8316-f509-45b1-8239-9c41d74247bb.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
**批量执行** ：等 API 流式接收完全结束，然后按顺序执行所有工具。简单可靠，但延迟高——第一个 Read 要等最后一个 tool\_use block 接收完才能开始。

**流式执行** （ `StreamingToolExecutor` ）：API 流式接收期间，每收到一个 tool\_use block 就立即开始执行。这是 Claude Code 的默认模式，也是性能优化的关键。

流式执行器的并发控制模型值得深入理解：

![流式工具执行器](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961332335-6664de91-b946-49ce-8233-7aa631c022af.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
3. **max\_output\_tokens 错误** ：等待恢复循环决定是否能继续
...
当模型自然停止（end\_turn）但 token 预算未用完时，系统会注入一条 nudge 消息让模型继续工作。这解决了一个实际问题：复杂任务（如大规模重构）可能需要模型输出超过默认 max\_output\_tokens 的内容。
...
![Tool 接口六个功能组](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961335192-6f644ebf-5f21-4cc2-a830-ecce686b8187.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![工具注册三种加载策略](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961338202-893e4611-ecca-4fb0-bb10-2a0428e32395.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![BashTool 8 层安全检查](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961340820-c563a6e4-3b7f-4aab-acb2-3b205b61c72b.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![权限模式](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961343514-43e28ae7-457d-4ba0-a2dc-294565cab443.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![权限判断主流程](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961346281-a1a3e386-75e3-4a63-b7ff-fe6b4456cc45.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![权限规则来源](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961349247-7f623a5f-03a9-4b5a-b5dd-474f06ac4ff4.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![三层协作架构](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961370749-59185632-c702-4982-8fac-87bb6772138b.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![AgentTool 路由设计](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961376231-78b6f447-47f2-45a7-9034-b083a7ae9288.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
**清理阶段** 的 8 项清理操作（MCP 断开、session hooks 清除、prompt cache 清理、文件状态缓存释放、Perfetto 追踪注销、transcript 映射清除、孤儿 todo 清除、后台 bash 终止）说明了子 agent 的资源占用——每个子 agent 都是一个完整的执行环境，需要精确的生命周期管理。

### 5.6 Fork Subagent：Prompt Cache 优化的极致

Fork 是实验性功能，代表了一种全新的子 agent 模式：继承父 agent 的完整对话历史和系统 prompt，只需要一个简短的指令。

核心优化目标是 **最大化 prompt cache 命中率** 。消息构建策略：保留父 agent 的完整 assistant message（所有 tool\_use blocks），为每个 tool\_use 生成相同的占位 tool\_result，在最后追加一个 per-child 的指令文本块。结果：只有最后一个文本块因 child 而异，前面的所有内容字节相同。多个 fork 并行启动时共享同一个 prompt cache 前缀。
...
![Team/Swarm 两种后端](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961379278-c34730ed-ea06-4803-8901-e9e59b7dfb3d.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![进程内 Teammate 运行器|201](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961382333-e45b70dc-d33d-49f2-a6c4-a575d841ae58.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![Teammate 通信路由](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961385321-18d8424b-0c7c-41c3-899f-d4f24e63a52b.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
这个原则的深层原因：worker 从零开始，没有 coordinator 的对话上下文。如果 coordinator 不综合研究结果就直接转发，worker 会缺乏关键信息。Coordinator 的核心价值就是 **综合** ——把多个 worker 的发现融合成精确的执行指令。
...
![Task 系统](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961388302-210733f9-923e-4ea4-9cd0-5dd3cea30ef2.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![权限传递规则](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961391833-1596da7e-8bc9-4c26-b2a5-2a0f2ad7b6f6.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![System Prompt 分段缓存架构](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961395416-d51845e1-6727-47dc-b29a-2b56ab4f952c.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![四层压缩策略](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961399231-3e5b4b0d-fc4a-41b7-bab4-43732e26a8cf.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![终端 UI 渲染管线](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961402072-141ded1e-8f92-479c-8e22-19a3e3634fc7.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
...
![MCP 四层架构](https://ata.atatech.org/router/file/redirect?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2026%2Fpng%2F1599908%2F1774961405290-642b84f0-e55b-40c0-8463-9b1ca8410615.png&kind=ARTICLE&process=image/auto-orient,1/resize,m_lfit,w_1600/quality,Q_80/format,webp)
