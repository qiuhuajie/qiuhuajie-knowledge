---
title: "殷浩详解DDD系列 第一讲 - Domain Primitive - ATA"
tags:
  - "系统架构"
  - "系统架构/领域驱动设计"
  - "领域驱动设计"
...
updated: 2026-04-16
---
# 一、殷浩详解DDD系列 第一讲 - Domain Primitive - ATA
## 1. 殷浩详解DDD系列 第一讲 - Domain Primitive
[LUAN GUANGMIAO(殷浩)](https://ata.atatech.org/users/11000205944)
2019-08-02发表2023-08-26更新27k浏览
** 字号
** 笔记
** 分享 **
3位翰林院成员推荐： [祥翼](https://ata.atatech.org/users/11000236615) [右军](https://ata.atatech.org/users/12000012184) [张建飞](https://ata.atatech.org/users/11000093134)
## 2. 写在最前面
...
| 入参为空 | ✔️ | ✔️ | ✔️ |
| 入参不符合要求（可能多个） | ✔️ | ✔️ | ✔️ |
假如一个方法有 $$ 个参数，每个参数有 $$ 个校验逻辑， **至少** 要有 $$ 个TC 。
如果这时候在该方法中加入一个新的入参字段 `fax` ，即使 `fax` 和 `phone` 的校验逻辑完全一致，为了保证TC覆盖率，也一样需要 $$ 个新的TC。
而假设有 $$ 个方法中都用到了 `phone` 这个字段，这 $$ 个方法都需要对该字段进行测试，也就是说整体需要：
$$
$$
...
- 首先 `PhoneNumber` 本身还是需要 $$ 个测试用例，但是由于我们只需要测试 **单一对象** ，每个用例的代码量会大大降低，维护成本降低。
- 每个方法里的每个参数，现在只需要覆盖为null的情况就可以了，其他的case不可能发生（因为只要不是null就一定是合法的）

所以，单个方法的TC从原来的 $$ 变成了今天的 $$ 。同样的，多个方法的TC数量变成了
$$
$$
这个数量一般来说要远低于原来的数量 $$ ，让测试成本极大的降低。
### 5.5 评估总结
...
### 7.2 使用Domain Primitive的三原则
- 让隐性的概念显性化
- 让隐性的上下文显性化
- 封装多对象行为

### 7.3 Domain Primitive和DDD里Value Object的区别
...
### 7.4 Domain Primitive和Data Transfer Object (DTO)的区别
...
### 8.1 第一步 - 创建Domain Primitive，收集所有DP行为
