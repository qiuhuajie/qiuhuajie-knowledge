...
1. `userdel 用户名`：删除用户，但是要保留家目录（**一般情况下建议保留用户家目录**）
    ...
     cool  test  tom
    ```
2. `userdel -r cool`：删除用户以及用户主目录（注意使用）
...
2. 当需要返回到原来用户时，使用exit指令
    ...
     [tom@hadoop1 ~]$
    ```
## 6. 查看当前用户/登陆用户：`who am i`（第一次登陆时使用的用户）
```Plain
...
2. 删除组：`groupdel 组名`
    ...
    ```
3. 增加用户时直接为其指定一个组：`useradd -g 组名 用户名`
...
2. **/etc/shadow** 文件
    1. 口令的配置文件
    2. 每行的含义： 登录名:加密口令:最后一次修改时间:最小时间间隔:最大时间间隔:警
        告时间:不活动时间:失效时间:标志
        ```Plain
