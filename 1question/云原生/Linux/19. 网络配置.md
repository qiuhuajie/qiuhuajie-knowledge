...
# 一、Linux网络配置原理图
**NAT网络配置：**
![[IMG-20260404031958918.png|800]]
# 二、查看网络IP和网关
1. 查看虚拟网络编辑器、修改ip地址、查看网关

    ![[IMG-20260404031958950.png|800]]
2. 查看windows环境的中VMnet8网络配置 (`ipconfig`指令)

    ![[IMG-20260404031958999.png|800]]
3. 查看Linux环境的中网络配置 (`ifconfig`指令)

    ![[IMG-20260404031959083.png|800]]
# 三、ping 测试主机之间网络连通性
1. 基本语法：
...
2. 注意：linux启动后会自动获取IP，缺点是每次自动获取的ip地址可能不一样

    ![[IMG-20260404031959165.png|800]]
## 2. ⭐第二种方法（指定固定的ip）服务器一定是要固定 IP
1. 直接修改配置文件来指定IP，并可以连接到外网（程序员推荐）
2. 编辑 `vi /etc/sysconfig/network-scripts/ifcfg-ens33`

    （这里的eth33就是指本机的网络设备-网卡）
    ![[IMG-20260404031959217.png|713]]
3. 将ip地址配置的静态的， ip地址为192.168.184.130（自定义）
    1. ens33文件原内容：
    ...
    2. 修改后的内容：
        ...
         DNS1=192.168.200.2
        ```
    3. 同时在虚拟机的虚拟网络编辑中将网关修改

        ![[IMG-20260404031959247.png|800]]
    4. 重启网络服务或者重启系统生效
        1. `service network restart`
        2. `reboot`
    5. 此时查看Windows vmnet8及Linux网络，都已经修改
...
2. 设置host映射
    ...
    2. 配置映射关系的文件所在位置：
        1. Windows："`C:\Windows\System32\drivers\etc\hosts`"（修改此处即域名劫持）
            ...
            ![[IMG-20260404031959354.png|592]]
            此时在命令行中已经可以使用主机名ping通了
            ![[IMG-20260404031959409.png|598]]
        2. Linux：`/etc/hosts`
...
2. DNS：Domain Name System域名系统，是互联网上作为**域名和IP地址**相互映射的**分布式文件数据库**
3. 应用实例：
    1. 先查看浏览器缓存中，有没有该域名的IP解析地址
    2. 若没有，检查DNS解析器缓存中，有没有该域名的IP解析地址
        1. 一般来说，当电脑第一次成功访问一个网站后，在一定时间段内，浏览器或者操作系统会缓存它的IP地址（DNS解析记录）
        2. 如在cmd窗口输入指令：
            1. `ipconfig /displaydns` //DNS域名解析缓存
            2. `ipconfig /flushdns` //手动清理DNS解析缓存
    3. 若没有，检查系统中hosts文件中，有没有该域名的IP解析地址
    4. 若没有，则到域名服务器DNS进行解析

        ![[IMG-20260404031959435.png|800]]
