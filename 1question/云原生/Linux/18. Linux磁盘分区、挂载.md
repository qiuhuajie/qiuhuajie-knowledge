...
1. 原理介绍：
    ...
    1. Linux采用了一种叫“载入”的处理方法，它的整个文件系统中包含了一整套的文件和目录，且将一个分区和一个目录联系起来。这时要载入的一个分区将使它的存储空间在一个目录下获得
    
1. 原理图：
    
    ![[IMG-20260404031958699.png|Untitled 122.png]]
    
1. 硬盘说明：
    
    1. Linux硬盘分IDE硬盘和SCSI硬盘，目前基本上是SCSI硬盘
    
    1. 对于`IDE`硬盘，驱动器标识符为“`hdx~`”,其中“hd”表明分区所在设备的类型，这里是指IDE硬盘了
        
        1. “`x`”为盘号（a为基本盘， b为基本从属盘， c为辅助主盘， d为辅助从属盘）
        
        1. “`~`”代表分区，前四个分区用数字1到4表示，它们是主分区或扩展分区，从5开始就是逻辑分区
        
        1. 例：hda3表示为第一个IDE硬盘上的第三个主分区或扩展分区；hdb2表示为第二个IDE硬盘上的第二个主分区或扩展分区
        
    
    1. 对于`SCSI`硬盘，则标识为“`sdx~`”， SCSI硬盘是用“sd”来表示分区所在设备的类型的，其余则和IDE硬盘的表示方法一样
...
1. **如何增加一块硬盘？**
    ...
    ![[IMG-20260404031958749.png|Untitled 1 76.png]]
    ...
    1. **注意：用命令行挂载重启后会失效**
        ...
        1. ==**永久挂载：**==通过修改`/etc/fstab`实现挂载
            ...
            ![[IMG-20260404031958864.png|Untitled 2 61.png]]
            
        
        1. 添加完成后 执行`mount –a` 即刻生效
