...
1. 如果复制到的目标文件夹内有同名的文件，如果**默认执行覆盖文件操作**怎么办
...
1. `mv /temp/movefile /targetFolder` (功能描述：移动文件) = 剪切
...
1. cat 只能浏览文件，而不能修改文件，为了浏览方便，一般会带上管道命令 `| 其他指令`
# **more 指令**
1. 基本语法：`more 要查看的文件`
1. 功能描述：是一个基于VI编辑器的文本过滤器，它以全屏幕的方式按页显示文本文件的内容
    

    ![[IMG-20260404031958217.png|Untitled 117.png]]
...
# **less 指令**
1. 基本语法：`less 要查看的文件`
1. 功能描述：用来分屏查看文件内容，它的功能与more指令类似，但是比more指令更加强大，支持各种显示终端。 less指令在显示文件内容时，并不是一次将整个文件加载之后才显示，而是**根据显示需要加载内容， 对于显示大型文件具有较高的效率**
1. 内置的若干快捷键：
...
1. 基本语法、功能描述：
    ...
    1. head -n 5 文件（功能描述：查看文件头5行内容， 5可以是任意行数）
...
1. `tail -n 5 文件` （功能描述：查看文件头5行内容， 5可以是任意行数）
1. `tail -f 文件` （功能描述：实时追踪该文档的所有更新）
...
1. 基本语法、功能描述：
    ...
    1. `ls -al >>文件` （功能描述：列表的内容**追加到**文件aa.txt的**末尾**）
    
    1. `cat 文件1 > 文件2` （功能描述：将文件1的内容**覆盖**到文件2）
...
1. 应用实例 ：
    ...
    ![[IMG-20260404031958275.png|Untitled 1 74.png]]
...
1. 基本语法、功能描述：
    ...
    ![[IMG-20260404031958332.png|Untitled 2 60.png]]
