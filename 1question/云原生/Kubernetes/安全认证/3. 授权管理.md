...
1. 如何实现授权
    ...
    1. 授权就是根据这些信息和授权策略进行比较，如果符合策略，则认为授权通过，否则会返回错误
    
1. API Server目前支持以下几种授权策略：
...
1. `**RBAC(Role-Based Access Control)**`基于角色的访问控制
    ...
    1. 角色：用于指定一组权限
        ...
        1. `**Role**`、`**ClusterRole**`
            ...
            ![[IMG-20260404031957669.png|Untitled 413.png]]

            
        
    
    1. 角色绑定：用于将角色（权限）赋予给对象
...
1. `**Role**`只能对命名空间内的资源进行授权，需要指定nameapce
...
1. `**ClusterRole**`可以对集群范围内资源、跨namespaces的范围资源、非资源类型进行授权
...
1. `**RoleBinding**`可以将同一namespace中的subject绑定到某个Role下，则此subject即具有该Role定义的权限
...
1. `**ClusterRoleBinding**`在整个集群级别和所有namespaces将特定的subject与ClusterRole绑定，授予权限
...
1. RoleBinding可以引用ClusterRole，对属于同一命名空间内ClusterRole定义的资源主体进行授权
