...
- 将pod对象从创建至终的这段时间范围称为pod的生命周期，它主要包含下面的过程：
    ...
    ![[IMG-20260404031955605.png|Untitled 523.png]]
...
## 1.2 Pod 五种状态
![[IMG-20260404031955681.png|Untitled 1 388.png]]
...
# 2. **Pod 的创建过程**
![[IMG-20260404031955758.png|Untitled 2 314.png]]
1. 用户通过 kubectl 命名发起 创建一个 Pod 的请求，并提交需要创建的 Pod 信息
1. 进入 API Server 中，将 Pod 相关信息存储到 etcd 中，然后返回确认信息至客户端
1. Scheduler
    
    1. Scheduler 会监听 API Server 中的 Watch 接口，查看是否有新的 Pod 被创建
    
    1. 如果有则从 etcd 中读取到新建 Pod 的信息，通过调度算法 把 Pod 调度到某个 node 节点上
    
    1. 并将 Pod 和对应节点绑定的信息交给 API Server，API Server 将绑定信息写到 etcd
    
1. 如果 Pod 被调度到 node02 节点上
    
    1. node02 节点上的 Kubelet 也会监听 API Server ，它监听的是 API Server 中是否有新的 Pod 被绑定到本节点
    
    1. 如果有，则从 etcd 中读取到 Pod 的绑定信息
        
        1. 调用 CNI 接口给 Pod 创建 Pod 网络
        
        1. 调用 CRI 接口去启动容器（通过 Docker 创建容器）
        
        1. 调用 CSI 进行存储卷的挂载
        
    
    1. 并将 Pod 的状态返回给 API Server
    
# 3. **Pod 的终止过程**
1. 用户向apiServer发送删除 Pod 对象的命令
1. apiServcer中的 Pod 对象信息会随着时间的推移而更新，在宽限期内（默认30s），pod被视为dead
1. apiServcer将 Pod 标记为 terminating 状态（修改pod在etcd的信息）
1. kubelet在监控（kubelet一直watch着etcd）到 Pod 对象转为terminating状态的同时启动 Pod 关闭过程
1. 端点控制器监控到 Pod 对象的关闭行为时将其从所有匹配到此端点的service资源的端点列表中移除
1. 如果当前 Pod 对象定义了preStop钩子处理器，则在其标记为terminating后即会以同步的方式启动执行
1. Pod 对象中的容器进程收到停止信号
1. 宽限期结束后，若pod中还存在仍在运行的进程，那么 Pod 对象会收到立即终止的信号
1. kubelet请求apiServer将此pod资源的宽限期设置为0从而完成删除操作，此时pod对于用户已不可见
# 4. **initContainers**
...
1. 初始化容器有两大特征：
    ...
    1. **（串行启动）**初始化容器必须按照定义的顺序执行，当且仅当前一个成功之后，后面的一个才能运行
    
1. 初始化容器有很多的应用场景，下面列出的是最常见的几个：
    
    - 提供主容器镜像中不具备的工具程序或自定义代码
    
    - 初始化容器要先于应用容器串行启动并运行完成，因此可用于延后应用容器的启动直至其依赖的条件得到满足
    
1. 示例：假设要以主容器来运行nginx，但是要求在运行nginx之前先要能够连接上mysql和redis所在服务器
    ...
    1. 动态查看pod：`STATUS`为 Init:0/2，两个初始化容器都没有启动
    ...
    1. 接下来新开一个shell，为当前服务器新增两个ip，观察pod的变化
    ...
    1. 上面动态查看pod的信息有更新：`STATUS` 逐步从 Init:0/2 变为 Running
...
1. kubernetes 在主容器的启动之后和停止之前提供了两个钩子函数：
    
    - **`post start`**：容器创建之后执行，如果失败了会重启容器
    
    - **`pre stop`** ：容器终止之前执行，执行完成之后容器将成功终止，在其完成之前会阻塞删除容器的操作
    
1. 钩子处理器支持使用下面三种方式定义动作：
...
1. 自愈的**默认实现方式：**
    ...
    1. 每个容器启动时都会执行一个进程，此进程由 Dockerfile 的 CMD 或 ENTRYPOINT 指定
    
    1. 如果进程退出时返回码非零，则认为容器发生故障，Kubernetes 就会根据 `restartPolicy` 重启容器
        ...
        >     
        >     1. 该进程会提供一个退出码（一个整数）表明它准备退出的原因。按照惯例，0用来表示正常的或者说“成功”的终止
        >     
        >     1. 这个退出码就是 退出状态码
        >     
        
    
1. ⭐==**但是！有不少情况是发生了故障，但进程并不会退出：**==
    
    1. 比如访问 Web 服务器时显示 500 内部错误，可能是系统超载，也可能是资源死锁
    
    1. **此时 httpd 进程并没有异常退出**
    
    1. 在这种情况下重启容器可能不是最直接最有效的解决方案，如何处理这类场景呢？
...
1. 示例一：以liveness probes为例，使用Exec方式探测
    ...
    1. 查看 Pod （发现nginx容器启动之后就进行了健康检查）
...
1. 示例二：以liveness probes为例，使用TCPSocket方式探测
...
1. 示例三：以liveness probes为例，使用HTTPGet方式探测
...
1. 当某个容器异常退出或者健康检查失败时，kubelet 将根据 `RestartPolicy` 的设置来进行相应的操作Pod的重启策略包括
    
    1. **`Always`**：当容器失效时，由 kubelet 自动重启该容器（默认策略）
    
    1. **`OnFailure`**：当容器异常退出（退出状态码非 0 时），由 kubelet 自动重启该容器
    
    1. **`Never`**：不论容器运行状态如何，kubelet 都不会重启该容器
...
1. 示例
    ...
    1. 多等一会，再观察pod的重启次数，发现一直是0，并未重启
